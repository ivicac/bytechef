...
2. **Quickstart step 2 is wrong about API keys.** It says to use the API key on `/api/embedded/internal` endpoints. The security code makes that impossible: internal paths never match the API-key converter's `/v{n}/{externalUserId}/` pattern, and even on success the principal has zero authorities (confirmed by Javadoc in `AutomationProjectCodeWorkflowAdminApiController` — this is why that controller re-mounts deploy on `/api/platform/v1`). API keys work only on public `/api/embedded/v1/{externalUserId}/…` routes. → One-paragraph doc fix.
3. **Branding "next step" points at nothing.** "Customize the connect dialog branding (see the SDK README)" — no branding/appearance surface exists in code, and the README doesn't cover it. Same class of problem: `white-label-execution.mdx` and `configuration-api.mdx` promise CSS theming, emails-from-your-domain, idempotency keys, cursor pagination — none exist in any spec or controller. → Mark "planned" or pull from nav; never show to a design partner as current capability.
