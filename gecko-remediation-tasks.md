...
- [~] **T25. Workspace/Git config, custom components, file entries, search providers** _(~25 findings)_ — membership-check workspace git-config and `getUserWorkspaces`; gate custom-component mutations; authenticate `/file-entries/{id}/content` or sign its URLs; scope all `SearchAssetProvider` `findAll()` calls.
  ...
  **Remaining:**
  - **`ConnectedUserGraphQlController`** (`connectedUser(id)`/`connectedUsers`) — a redundant GraphQL path NOT used by the client (the client uses the now-gated REST `ConnectedUserApiController`). It calls `ConnectedUserService.getConnectedUser(long)` directly, which is shared with embedded management facades (`ConnectedUserConnectionFacadeImpl`, `IntegrationInstanceFacadeImpl`, `ConnectedUserProjectFacadeImpl`), so service-tier gating needs per-caller analysis; the clean fix is to route the controller through the gated `ConnectedUserFacade` (entity-vs-DTO return-type adjustment) or retire it.
