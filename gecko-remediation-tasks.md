...
- [~] **T23. Integration instances & embedded `externalUserId`/`X-Instance-Id` trust** _(~35 findings)_ — verify that `externalUserId` (URL path) and `X-Instance-Id`/`x-instance-id` (headers) match the authenticated principal before resolving connections/instances/workflows/tools/actions; fix the tautological ownership checks.
  ...
  **Remaining:** a per-finding audit of the embedded facades (`ConnectedUserIntegration(Instance)FacadeImpl`, `UnifiedApiFacadeImpl` integrationInstanceId ownership, `IntegrationWorkflowFacadeImpl`) for any residual tautological owner checks now that the principal-match is enforced upstream; the internal `embedded-configuration-rest-impl/ConnectionApiController` (workflow-builder surface, separate auth) was out of the public-API scope.
