...
- [~] **T24. Jobs, executions, logs, approval tasks/forms, notifications, eval, app-events** _(~40 findings)_ — add per-resource authorization to job/execution APIs, log-file GraphQL readers, approval task mutations and approval-form/token endpoints, notification CRUD, AI-agent eval (scenario/judge/result/run), and AppEvent CRUD; sign capability tokens (approval/trigger-form) with HMAC instead of plain base64.
  ...
  **Remaining (design-required):**
  - **Approval / trigger-form HMAC signing:** `JobResumeId` (live, `ApprovalFormFacadeImpl`/`TriggerFormApiController`) and `ApprovalId` (deprecated, `ApprovalController`) are plain `base64(tenantId:jobId:uuid[:approved])` capability tokens minted into anonymous email links — unsigned, so `jobId`/`approved` are forgeable. These are anonymous endpoints (no `SecurityContext`), so the only fix is to **HMAC-sign the token**, mirroring the established `FileEntryTokens.toSignedToken` pattern (HMAC-SHA256, key derived from `EncryptionKey` via a domain-separation label, `v1.<payload>.<sig>` format, accept-legacy flag for in-flight tokens during migration). Warrants a short spec/plan before implementing given crypto + backward-compat.
...
- [~] **T25. Workspace/Git config, custom components, file entries, search providers** _(~25 findings)_ — membership-check workspace git-config and `getUserWorkspaces`; gate custom-component mutations; authenticate `/file-entries/{id}/content` or sign its URLs; scope all `SearchAssetProvider` `findAll()` calls.
  ...
  **Remaining:**
  - **Embedded connected-user / integration management** (`ConnectedUserGraphQlController`/`ConnectedUserApiController` list+get, `IntegrationGraphQlController`, `IntegrationTagApiController`/`IntegrationInstanceConfigurationTagApiController`) — real IDOR (any user reads connected users / integration config), but `ConnectedUserService` is heavily runtime-shared (webhook dispatch in `AppEventTriggerApiController`/`RequestTriggerApiController`, execution via `ConnectionIdHelper`). Gating needs per-overload runtime-sharing analysis (the numeric `getConnectedUser(long)` / paged `getConnectedUsers` management overloads vs the `(externalUserId, environment)` runtime overload) or a per-controller `Tenant ADMIN` facade. Not yet done.
  - **Search providers** — same residual class as T22: `SearchAssetProvider.findAll()` calls return cross-workspace results; fix by workspace-scoping the underlying query.
  - **`WorkspaceApiController.getUserWorkspaces`** — deliberately left to the user (their live RBAC/custom-role area; see project memory).
