...
- [~] **T25. Workspace/Git config, custom components, file entries, search providers** _(~25 findings)_ — membership-check workspace git-config and `getUserWorkspaces`; gate custom-component mutations; authenticate `/file-entries/{id}/content` or sign its URLs; scope all `SearchAssetProvider` `findAll()` calls.
  ...
  **Search providers — foundation + 5/10 done** (spec Part B, implemented as a safer additive variant): `SearchResult.workspaceId()` (nullable) + `AutomationSearchFacadeImpl` resolves the caller's accessible workspaces once in the authenticated calling thread (`getUserWorkspaces(currentUserId)`) and drops any result whose `workspaceId` is not accessible (null = passed through, so providers scope incrementally with no atomic SPI break; aggregator filter unit-tested). Scoped: Workflow/Project/ProjectDeployment (project → `workspace_id`), DataTable/AssetFile (hardcoded `workspaceId=1L` stamped so non-workspace-1 callers are dropped). **Remaining 5 providers** (null `workspaceId` today = current behavior, each a safe independent follow-up): `ConnectionSearchAssetProvider` (platform; needs `workspace_connection`), `KnowledgeBaseSearchAssetProvider`/`KnowledgeBaseDocumentSearchAssetProvider` (platform; needs `workspace_knowledge_base`), `ApiCollectionSearchAssetProvider`/`ApiEndpointSearchAssetProvider` (EE api-platform) — each needs a cross-module workspace-relation lookup to stamp `workspaceId`.
