...
public class SpringAIAgent extends LocalAgent {
     ...
     *
     * <p>
     * Symmetric to {@link #toolContext(RunAgentInput)} — both let subclasses inject per-request data without
     * touching the chat client construction. Use cases include per-conversation tool bindings (where the
     * candidate set depends on which conversation the request targets) and per-tenant feature gating.
     * </p>
     *
     * @param input the current run input
        ...
        if (evt.hasToolCalls()) {
            ...
            evt.getResult().getOutput().getToolCalls()
                .forEach(toolCall -> {
                    ...
                    // Do NOT emit toolCallStart/Args/End events here. The AgUiFunctionToolCallback wrapper
                    // installed in getChatRequest() is the single source of tool-call lifecycle events: it
                    // emits start/args/end AND the matching result event, all under one consistent toolCallId.
                    // Emitting start/args/end here too — keyed by the model's toolCall.id() rather than that
                    // wrapper's id — produced a second, duplicate tool-call card on the client that never
                    // received a result event, surfacing on the UI as "Run finished with no result event".
    ...
    private ChatClient.ChatClientRequestSpec getChatRequest(RunAgentInput input, String content, String messageId, List<BaseEvent> deferredEvents, SystemMessage systemMessage, AgentSubscriber subscriber) throws AGUIException {
        ...
        // Concat static (builder-time) callbacks with any per-request additions from subclass hooks. Done in
        // one .toolCallbacks(...) call so Spring AI sees a single coherent list and the AgUi event-recording
        // wrapper applies uniformly. Empty union case still skips the call to keep the prompt lean.
