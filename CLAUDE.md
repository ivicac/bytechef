...
- If the entity later needs to be shared beyond its owning workspace, add a `visibility` enum modeled
  on `ConnectionVisibility` (`PRIVATE < WORKSPACE < ORGANIZATION`, gated `canTransitionTo` transitions,
  ordinals pinned by a stability test). Sharing is **additive to a column** — it does not need a
  relation table, and it expresses ownership + reach, which membership rows cannot.
...
or `ORGANIZATION` (cross-workspace).

- **CE**: only `PRIVATE` is reachable. `ConnectionFacadeImpl.create()` forces visibility to PRIVATE
  whenever the running edition is not `EE`. The visibility selector, scope badge, promote/demote
  menu items, and demote-confirm dialog are all hidden in CE.
- **EE**: visibility selector appears in the connection create dialog (only `PRIVATE` for non-admins,
  ...
  promote/demote actions to admins; the connection's creator can also demote a
...
- `promoteConnectionToWorkspace(workspaceId, connectionId)` — PRIVATE → WORKSPACE.
