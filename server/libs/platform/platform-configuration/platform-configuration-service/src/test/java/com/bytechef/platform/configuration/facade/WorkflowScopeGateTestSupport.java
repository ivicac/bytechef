...
/**
 * Stands in for {@code AutomationMethodSecurityExpressionHandler} so this module's
 * {@code hasWorkflowScopeInEnvironment(...)} {@code @PreAuthorize} expressions can be evaluated in a test context.
 * <p>
 ...
 * own arguments -- while the function's own semantics (ordinal resolution, fail-closed on an unidentifiable
 * environment, skip-mode ordering) are pinned by {@code AutomationMethodSecurityExpressionRootTest} beside the
 * production implementation. This is the same pattern already used for {@code WorkflowTestApiController} in
 * {@code platform-workflow-test-rest}.
 *
 * @author Ivica Cardic
 */
final class WorkflowScopeGateTestSupport {
    ...
    /**
     * Records the arguments the SpEL expression actually passed. {@link #permit(boolean)} answers every call the same
     * way regardless of environment, which is enough for a plain allow/deny wiring test.
     * {@link #permitOnlyForEnvironment(Long)} instead answers based on the {@code environmentId} argument alone, so a
     * single fixture can stand in for "a member holding the scope in one environment only" across two calls that name
     * different environments -- proving the gate's per-environment branch is actually exercised rather than a constant
    ...
    static final class GateRecorder {
        ...
        private boolean environmentConditional;
