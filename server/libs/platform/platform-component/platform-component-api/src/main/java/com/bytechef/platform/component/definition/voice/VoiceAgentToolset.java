...
/**
 * The tools a voice agent may call. {@link #call} never throws: an unknown name, a failing tool or a tool that needs an
 * approval the session cannot host all come back as the result string, so the provider can forward it and the agent can
 * say so aloud.
 *
 * @author Ivica Cardic
 */
