 ...
 *
 * <p>
 * The module's test harness cannot stand up a real synchronous webhook run: that requires the in-process
 * {@code JobSyncExecutor}, a task handler and a deployed workflow that produces a {@code WEBHOOK_RESPONSE}-tagged task
 * output. How the executor assembles the reply from the engine's completion callback is covered by
 * {@code WebhookWorkflowExecutorTest}. What this test pins is the unit of behaviour the controller itself owns: it
 * returns a {@link CompletableFuture}, so Spring MVC dispatches the response through the async-servlet path, then
 * either renders the tagged {@code WebhookResponse} or falls back to {@code ResponseEntity.ok(outputs)}. The executor's
 * future is stubbed to resolve exactly as the real path resolves it once the run finishes.
 *
 * @author Ivica Cardic
 */
