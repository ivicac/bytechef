 ...
 *
 * <p>
 * {@link WebhookWebSocketHandler} is registered at the production webhook upgrade path ({@code /webhooks/(id)/wss}) for
 * browser voice sessions; an upgrade without a session token is refused. Keeps the relative endpoint identical to the
 * REST webhook path.
 *
 * <p>
 * {@link WorkflowTestWebSocketHandler} is registered at the in-editor voice test path
 * ({@code /internal/workflow-tests/(id)/wss}), gated by the standard session cookie. It resolves the draft workflow and
 * starts its {@code browser/v1/voiceSession} trigger's Voice Agent cluster element against the live audio stream.
 *
 * <p>
 * Both track their sessions in {@link VoiceSessionRegistry}.
 *
 * @author ByteChef
 */
@Configuration
