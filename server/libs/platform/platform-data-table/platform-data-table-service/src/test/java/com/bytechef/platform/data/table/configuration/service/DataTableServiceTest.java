...
class DataTableServiceTest {
    ...
    void testCreateTableExecutesTheSharedCreateStatement() {
        ...
        dataTableService.createTable(
            "conversations", null, columnSpecs, 0, PlatformType.AUTOMATION, Optional.empty());
    ...
    void testDropTableShouldDeleteMetadataWhenNoPhysicalTablesRemain() {
        givenSharedRegistryRow();

        when(jdbcTemplate.query(anyString(), any(PreparedStatementSetter.class), any(RowMapper.class)))
            .thenReturn(List.of());

        dataTableService.dropTable("mytable", 1L, PlatformType.AUTOMATION, Optional.empty());
