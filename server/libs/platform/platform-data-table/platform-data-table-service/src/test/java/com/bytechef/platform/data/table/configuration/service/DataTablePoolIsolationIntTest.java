...
class DataTablePoolIsolationIntTest {
    ...
    void testDroppingOnePoolsLastInstanceLeavesTheOtherPoolsRegistryRowIntact() {
        ...
        dataTableService.dropTable("dualpool", ENVIRONMENT_ID, PlatformType.EMBEDDED);
    ...
    /**
     * The webhook registry is keyed on the resolved table, so the pool separation has to survive into it: a
     * registration made in one pool must not fire for the other pool's table of the same name.
     *
     * <p>
     * Asserted here against a real schema and real resolution rather than only against a mocked {@code fetchDataTable},
     * because the claim is about which registry row a name resolves to -- and a stubbed resolution can be made to agree
     * with either answer.
     */
    @Test
    void testAWebhookRegisteredInOnePoolNeverFiresForTheOtherPoolsTableOfTheSameName() {
        dataTableService.createTable(
            "hookedpool", null, List.of(new ColumnSpec("title", ColumnType.STRING)), ENVIRONMENT_ID,
            PlatformType.AUTOMATION);
        dataTableService.createTable(
            "hookedpool", null, List.of(new ColumnSpec("title", ColumnType.STRING)), ENVIRONMENT_ID,
            PlatformType.EMBEDDED);
        ...
        dataTableWebhookService.addWebhook(
            resolve("hookedpool", PlatformType.EMBEDDED), "https://example.test/embedded",
            DataTableWebhookType.RECORD_CREATED);

        assertThat(urlsOf(resolve("hookedpool", PlatformType.AUTOMATION)))
            .as("an embedded registration must not fire for the automation table of the same name")
            .isEmpty();
        assertThat(urlsOf(resolve("hookedpool", PlatformType.EMBEDDED)))
            .containsExactly("https://example.test/embedded");
    }

    private DataTableRef resolve(String baseName, PlatformType platformType) {
        return dataTableService.fetchDataTableResolution(baseName, ENVIRONMENT_ID, platformType, Optional.empty())
            .orElseThrow()
            .dataTableRef();
    }
    ...
    private List<String> baseNames(PlatformType platformType) {
        return dataTableService.listTables(ENVIRONMENT_ID, platformType)
            .stream()
