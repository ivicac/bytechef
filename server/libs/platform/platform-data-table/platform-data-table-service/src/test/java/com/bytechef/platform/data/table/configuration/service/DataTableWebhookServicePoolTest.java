 ...
 * table; it stopped being so the moment an account could own an {@code invoices} of its own alongside the vendor's. A
 * name-keyed registry binds both registrations to the vendor's row, so the account's rows are POSTed to the vendor's
 * URL and the account's own registration never fires.
 *
 * <p>
 * Keyed on the resolved table instead, registration and delivery go through the same lookup and therefore meet: the
 * pool is in the ref, and so is the owner.
 *
 * @author Ivica Cardic
 */
...
class DataTableWebhookServicePoolTest {
    ...
    private static final long ACCOUNT_A_TABLE_ID = 3L;
    ...
    private static final DataTableRef AUTOMATION_TABLE =
        DataTableRef.shared("invoices", ENVIRONMENT_ID, PlatformType.AUTOMATION);
    private static final DataTableRef VENDOR_TABLE =
        DataTableRef.shared("invoices", ENVIRONMENT_ID, PlatformType.EMBEDDED);

    /**
     * Account A resolving a base name it has no table of its own for: the vendor's physical table, addressed by a run
     * acting for account A. The pairing the ref carries two owners for, and the only case the registration owner
     * decides.
     */
    private static final DataTableRef SHARED_TABLE_AS_ACCOUNT_A =
        new DataTableRef("invoices", ENVIRONMENT_ID, PlatformType.EMBEDDED, null, ACCOUNT_A_OWNER);
    ...
    void testAddingAWebhookBindsToTheResolvedTableAndNotTheName() {
        givenTheNameExistsInBothPoolsAndForOneAccount();
        givenTheRepositoryAssignsIds();

        long webhookId = dataTableWebhookService.addWebhook(
            ACCOUNT_A_TABLE, "https://example.test/a", DataTableWebhookType.RECORD_CREATED);
        ...
        assertThat(savedDataTableWebhook.getDataTableId())
            .as("the registration must bind to the account's own table, not the vendor's of the same name")
            .isEqualTo(ACCOUNT_A_TABLE_ID);

        assertThat(resolvedDataTableRef())
            .as("the registration must be resolved through the ref it was given, owner and pool intact")
            .isEqualTo(ACCOUNT_A_TABLE);
    }
    ...
    void testARegistrationIsStampedWithTheRefsRunOwner() {
        ...
        dataTableWebhookService.addWebhook(
            SHARED_TABLE_AS_ACCOUNT_A, "https://example.test/a", DataTableWebhookType.RECORD_CREATED);
    ...
    void testASharedTableLookupReturnsOnlyTheAskingAccountsRegistrations() {
        ...
        List<Webhook> webhooks = dataTableWebhookService.listWebhooks(SHARED_TABLE_AS_ACCOUNT_A);
    ...
    /**
     * The delivery half of the same regression: an event on the account's table must look up the account's
     * registrations, never the vendor's, even though the two tables share a base name and a pool.
     */
    @Test
    void testListingWebhooksForAnOwnedTableNeverReachesTheVendorsRegistrations() {
        givenTheNameExistsInBothPoolsAndForOneAccount();

        when(dataTableWebhookRepository.findByDataTableIdAndEnvironment(
            ACCOUNT_A_TABLE_ID, Environment.DEVELOPMENT.ordinal()))
                .thenReturn(
                    List.of(dataTableWebhook(9L, ACCOUNT_A_TABLE_ID, "https://example.test/a", ACCOUNT_A_OWNER)));

        List<Webhook> webhooks = dataTableWebhookService.listWebhooks(ACCOUNT_A_TABLE);
        ...
        verify(dataTableWebhookRepository, never())
            .findByDataTableIdAndEnvironment(VENDOR_TABLE_ID, Environment.DEVELOPMENT.ordinal());
    }
    ...
    void testAnUnregisteredTableHasNoWebhooks() {
        when(dataTableService.fetchDataTable(ACCOUNT_A_TABLE)).thenReturn(Optional.empty());

        assertThat(dataTableWebhookService.listWebhooks(ACCOUNT_A_TABLE)).isEmpty();
    }
    ...
    /**
     * Three registry rows claim the name {@code invoices}: one per pool, plus the account's own copy alongside the
     * vendor's. Only a lookup that carries both the pool and the owner picks the right one.
     */
    private void givenTheNameExistsInBothPoolsAndForOneAccount() {
        ...
        lenient().when(dataTableService.fetchDataTable(ACCOUNT_A_TABLE))
            .thenReturn(Optional.of(dataTable(ACCOUNT_A_TABLE_ID)));

        // The fallback: account A addressing the vendor's row, which is the same registry row the vendor addresses.
        // Two refs, one row -- which is why the registration owner and not the row is what tells them apart.
        lenient().when(dataTableService.fetchDataTable(SHARED_TABLE_AS_ACCOUNT_A))
            .thenReturn(Optional.of(dataTable(VENDOR_TABLE_ID)));
    }
