...
class DataTableRowServiceWebhookEventTest {
    ...
    private void givenARowIsAffected(long rowId) {
        when(
            jdbcTemplate.query(
                anyString(), any(PreparedStatementSetter.class), ArgumentMatchers.<RowMapper<Object>>any()))
                    .thenAnswer(invocation -> {
                        ...
                        // The delete returns the row it removed, so the event can carry its values; every other
                        // statement this fixture answers is a catalog read.
                        if (sql.startsWith("DELETE FROM")) {
                            return List.of(new DataTableRow(rowId, Map.of("title", "invoice")));
                        }
