 ...
 * which after per-account tables no longer identifies one table: the vendor's {@code orders} and account 42's
 * {@code orders} are two tables sharing a name.
 *
 * <p>
 * The table alone is not enough, though, and briefly this interface said it was. A shared table has no owner and holds
 * the rows of every account in the pool, so keying on it fires every registration on it for every row written into it.
 * A registration therefore also carries the owner of the run that made it, and delivery is scoped by both: see
 * {@link Webhook#receivesRowsWrittenBy}.
 *
 * @author Ivica Cardic
 */
