 ...
 *
 * <p>
 * It once carried no owner, on the premise that the table it hangs off belongs to exactly one account so an owner here
 * could only agree with the table's or contradict it. Row-level ownership made that premise false: a table belongs to
 * nobody and holds rows belonging to many accounts, so the table's identity says nothing about whose row an event is
 * about. Without an owner here every registration fired for every row written into the table, and one account's row
 * values were POSTed to another's registered URL.
 *
 * <p>
