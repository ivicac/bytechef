...
class DataSyncVisibilityProvidersTest {
    ...
    void setUp() {
        ...
        dataSync.setProjectWorkflowUuid(PROJECT_WORKFLOW_UUID);

        when(dataSyncRepository.findById(DATA_SYNC_ID)).thenReturn(Optional.of(dataSync));
        when(dataSyncRepository.findProjectIdByProjectWorkflowUuid(PROJECT_WORKFLOW_UUID))
    ...
    void testDataSyncWithoutProjectFailsClosed() {
        ...
        orphanDataSync.setProjectWorkflowUuid(orphanProjectWorkflowUuid);

        when(dataSyncRepository.findById(4L)).thenReturn(Optional.of(orphanDataSync));
        when(dataSyncRepository.findProjectIdByProjectWorkflowUuid(orphanProjectWorkflowUuid))
    ...
    @Test
    void testSyncWhoseWorkflowIsGoneResolvesToNoRecord() {
        UUID missingProjectWorkflowUuid = UUID.randomUUID();

        DataSync dataSync = new DataSync();

        dataSync.setId(6L);
        dataSync.setProjectWorkflowUuid(missingProjectWorkflowUuid);

        when(dataSyncRepository.findById(6L)).thenReturn(Optional.of(dataSync));
        when(dataSyncRepository.findProjectIdByProjectWorkflowUuid(missingProjectWorkflowUuid))
            .thenReturn(Optional.empty());

        assertThat(dataSyncVisibilityProvider.fetchVisibility(6L)).isEmpty();
    }
