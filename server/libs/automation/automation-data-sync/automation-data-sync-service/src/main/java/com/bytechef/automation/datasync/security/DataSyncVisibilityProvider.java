...
/**
 * A Data Sync has no visibility of its own: it is exactly as visible as the project it keeps its generated workflow in,
 * so the record returned here is that PROJECT's and its grants are looked up under {@code "Project"} — the same
 * inheritance the sibling AI Agent feature's visibility provider expresses for its own parent.
 *
 * <p>
 ...
 * this one. The project is read through the sync's {@code project_workflow_uuid}; when no project workflow carries it
 * the lookup is empty and the provider answers with no record, so the by-id gates deny rather than treat the sync as
 * unrestricted.
 *
 * <p>
...
public class DataSyncVisibilityProvider implements ResourceVisibilityProvider {
    ...
    public Optional<VisibilityRecord> fetchVisibility(long id) {
        return dataSyncRepository.findById(id)
            ...
            .flatMap(dataSyncRepository::findProjectIdByProjectWorkflowUuid)
