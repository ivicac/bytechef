 ...
 * a colleague has withheld, unless they hold a grant. The answer comes from the agent's project rather than from a
 * second column on {@link AiAgent} — an agent's generated workflow is not reachable as a capability separate from the
 * agent, so the two can never need to diverge, and one question is kept to one record.
 *
 * <p>
