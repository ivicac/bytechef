...
/**
 * Pins the {@code @PreAuthorize} expression on every method of {@link AiAgentFacadeImpl} — the six keyed on a
 * {@code workspaceId}, the eleven keyed on an agent id, the four keyed on a channel or element id, and the one that is
...
class AiAgentFacadeAuthorizationTest {
     ...
     * adds a generated draft workflow to a project in the target workspace, creating that project when none is named.
    ...
    void testImportAgentRequiresWorkspaceAgentCreator() {
        assertExpression(AGENT_CREATE_BY_WORKSPACE_ID, "importAgent", long.class, String.class, Long.class);
    }

    /**
     * Copying a project's agents provisions fresh agents and draft workflows in the workspace, the same write
     * {@link #testCreateAgentRequiresWorkspaceAgentCreator()} gates.
     */
    @Test
    void testCopyProjectAgentsRequiresWorkspaceAgentCreator() {
        assertExpression(AGENT_CREATE_BY_WORKSPACE_ID, "copyProjectAgents", long.class, long.class, long.class);
    }
        ...
        assertThat(
            workspaceKeyedMethods.stream()
                ...
                .toList())
                    ...
                    .containsExactly(
                        "copyProjectAgents", "createAgent", "getAgentDeployments", "getAgents",
     ...
     * this facade that removes the agent and every published version of its generated workflow with it. The scope is
     * EDITOR-rank like the rest of the family, so this is a naming distinction rather than a privilege one today — but
     * the two are separable in a custom role, and a role built to let someone tune agents without being able to destroy
     * them is exactly the distinction {@code isAuthenticated()} could not express.
     */
    @Test
    ...
    /**
     * Runs from every project publish, through {@code ProjectPublishPreListener}, so it is keyed on the project rather
     * than on any one agent: it rewrites the draft workflow of every agent in the project.
     */
    @Test
    ...
    /**
     * Runs from every project delete, through {@code ProjectDeleteEventListener}, and takes the same scope the project
     * delete itself does.
     */
    @Test
    ...
    @Test
    void testUpdateAgentFromExportRequiresAgentEdit() {
        assertExpression(AGENT_EDIT_BY_ID, "updateAgentFromExport", long.class, String.class);
    }
