 ...
 * {@link com.bytechef.automation.ai.agent.facade.AiAgentFacadeImpl} needs to create, regenerate, and publish an agent
 * inside its project: {@code ProjectService}, {@code ProjectWorkflowService}, {@code ProjectDeploymentService},
 * {@code ProjectDeploymentWorkflowService}, {@code WorkflowService} (JDBC-backed), and — for the draft's
 * test-connection sync — {@code WorkflowTestConfigurationService}. {@code TriggerDefinitionService} is mocked rather
 * than wired for real: its only callers are
...
@Import({
    ...
    ProjectWorkflowCallback.class, WorkflowTestConfigurationServiceImpl.class
})
@Configuration
