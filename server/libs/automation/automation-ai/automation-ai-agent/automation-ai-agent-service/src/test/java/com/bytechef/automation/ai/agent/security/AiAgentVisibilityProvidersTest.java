 ...
 * their own, so every provider here answers with the backing PROJECT's record under the {@code "Project"} type.
 *
 * <p>
...
class AiAgentVisibilityProvidersTest {
    ...
    private static final UUID PROJECT_WORKFLOW_UUID = UUID.randomUUID();
    ...
    void setUp() {
        ...
        agent.setProjectWorkflowUuid(PROJECT_WORKFLOW_UUID);

        when(aiAgentRepository.findById(AGENT_ID)).thenReturn(Optional.of(agent));
        when(aiAgentRepository.findProjectIdByProjectWorkflowUuid(PROJECT_WORKFLOW_UUID))
    ...
    void testAgentWithoutProjectFailsClosed() {
        ...
        UUID orphanProjectWorkflowUuid = UUID.randomUUID();

        orphanAgent.setId(4L);
        orphanAgent.setProjectWorkflowUuid(orphanProjectWorkflowUuid);

        when(aiAgentRepository.findById(4L)).thenReturn(Optional.of(orphanAgent));
        when(aiAgentRepository.findProjectIdByProjectWorkflowUuid(orphanProjectWorkflowUuid))
            .thenReturn(Optional.of(404L));
    ...
    /**
     * The agent's project is read through its generated workflow, so an agent whose workflow row is gone has no project
     * to inherit a record from and must fail closed the same way.
     */
    @Test
    void testAgentWithoutProjectWorkflowFailsClosed() {
        AiAgent orphanAgent = new AiAgent();

        UUID missingProjectWorkflowUuid = UUID.randomUUID();

        orphanAgent.setId(8L);
        orphanAgent.setProjectWorkflowUuid(missingProjectWorkflowUuid);

        when(aiAgentRepository.findById(8L)).thenReturn(Optional.of(orphanAgent));
        when(aiAgentRepository.findProjectIdByProjectWorkflowUuid(missingProjectWorkflowUuid))
            .thenReturn(Optional.empty());

        assertThat(aiAgentVisibilityProvider.fetchVisibility(8L)).isEmpty();
    }
