 ...
 * draft ↔ published relationship of the agent's project.
 *
 * @param agent                the agent row
 * @param projectId            the project the agent belongs to — the project of its generated workflow, resolved from
 *                             {@code project_workflow} since {@code ai_agent} keeps no project column
...
public record AiAgentDTO(
    AiAgent agent, long projectId, List<AiAgentChannel> channels, List<AiAgentElement> elements,
    boolean unpublishedChanges,
    int lastPublishedVersion, @Nullable Instant publishedDate, ResourceVisibility visibility) {
