...
/**
 * One entry of an agent's version history — a {@code ProjectVersion} of the agent's project, which is what publishing
 * an agent actually mints. Exposed rather than the raw {@code ProjectVersion} so the agent surface never has to name
 * the backing project, which is an implementation detail everywhere else.
 *
