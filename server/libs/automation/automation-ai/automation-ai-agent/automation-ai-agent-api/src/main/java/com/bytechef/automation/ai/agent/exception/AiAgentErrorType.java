...
public class AiAgentErrorType extends AbstractErrorType {
    ...
    /** Deleting an agent whose own generated workflow is enabled in a {@code project_deployment}. */
    ...
    /**
     * A sharing operation named an agent that does not exist, or one whose {@code workspace_id} is null. No longer
     * raised: agents are shared through their project's sharing, so the agent-keyed sharing facade that raised it is
     * gone. The code stays reserved so it is never reused for a different error.
     */
