...
class DbSchedulerUiSecurityIntTest {
    ...
    static class DbSchedulerUiSecurityIntTestConfiguration {
        ...
        /**
         * Stubbed rather than the real component: the real one generates a key and persists it under the user's home
         * directory when it is constructed, while this test only needs the filter chain to build. The key has to be in
         * place before the context refreshes, which is why it is a bean rather than a {@code @MockitoBean} stubbed per
         * test.
         */
        @Bean
        RememberMeKey rememberMeKey() {
            ...
            when(rememberMeKey.getKey()).thenReturn("test-remember-me-key");
