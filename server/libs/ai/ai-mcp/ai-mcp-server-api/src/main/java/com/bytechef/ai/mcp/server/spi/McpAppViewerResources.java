...
/**
 * Canonical {@code ui://bytechef/<name>} resource identifiers for the read-only MCP App viewers. Shared between the
 * management server (which registers the resources) and the tool contributors (which reference them in their
 * {@link McpAppUiDescriptor}s) so the two never drift.
 *
 * @author Ivica Cardic
 */
public final class McpAppViewerResources {
