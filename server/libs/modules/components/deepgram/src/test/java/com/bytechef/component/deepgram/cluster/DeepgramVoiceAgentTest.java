 ...
 * @author Ivica Cardic
 */
class DeepgramVoiceAgentTest {

    @Test
    void testToVoiceEventMapsUserConversationTextToFinalTranscript() {
        Map<String, Object> voiceEvent = DeepgramVoiceAgent.toVoiceEvent(
    ...
    void testToVoiceEventMapsUserStartedSpeakingToSpeechStart() {
        Map<String, Object> voiceEvent = DeepgramVoiceAgent.toVoiceEvent(Map.of("type", "UserStartedSpeaking"));
    ...
    void testToVoiceEventTreatsMissingContentAsEmptyText() {
        ...
        Map<String, Object> voiceEvent = DeepgramVoiceAgent.toVoiceEvent(deepgramMessage);
    ...
    void testToVoiceEventReturnsNullForControlFrames() {
        assertThat(DeepgramVoiceAgent.toVoiceEvent(Map.of("type", "Welcome"))).isNull();
        assertThat(DeepgramVoiceAgent.toVoiceEvent(Map.of("type", "SettingsApplied"))).isNull();
    }

    @Test
    void testToVoiceEventReturnsNullWhenTypeMissing() {
        assertThat(DeepgramVoiceAgent.toVoiceEvent(Map.of("role", "user"))).isNull();
    }

    @Test
    void testFunctionCallRequestIsMappedToToolCall() {
        Map<String, Object> deepgramMessage = Map.of(
            "type", "FunctionCallRequest", "functions",
            List.of(Map.of("id", "call-1", "name", "lookupOrder", "arguments", "{}")));

        assertThat(DeepgramVoiceAgent.toVoiceEvent(deepgramMessage)).isNull();
    }
}
