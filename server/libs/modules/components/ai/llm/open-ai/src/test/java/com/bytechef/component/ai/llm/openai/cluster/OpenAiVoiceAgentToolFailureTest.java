 ...
 * OpenAI needs a {@code function_call_output} regardless, and the browser needs a {@code tool_result} it can show.
 *
 * @author Ivica Cardic
 */
@ExtendWith(ObjectMapperSetupExtension.class)
class OpenAiVoiceAgentToolFailureTest {
    ...
    void testToolCallStillAnswersWhenTheToolThrows() throws Exception {
        ...
        WebSocketHandler handler = OpenAiVoiceAgent.of(connector)
            .getElement()
            .apply(
                ParametersFactory.create(Map.of()), ParametersFactory.create(Map.of("token", "sk-test")),
        ...
        assertThat(socket.sentTexts()).anySatisfy(text -> {
            assertThat(text).contains("\"type\":\"function_call_output\",\"call_id\":\"call-1\"");
            assertThat(text).contains("\"output\":\"boom\"");
        });
        assertThat(socket.sentTexts()).anySatisfy(text -> assertThat(text).contains("\"type\":\"response.create\""));
    }
