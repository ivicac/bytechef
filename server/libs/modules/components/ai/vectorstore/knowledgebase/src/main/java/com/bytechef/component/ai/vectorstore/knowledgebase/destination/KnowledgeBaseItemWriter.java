...
public class KnowledgeBaseItemWriter implements ItemWriter {
     ...
     * the full gate: the id must name a knowledge base in a pool that owner may read, which is EMBEDDED alone, and the
     * owner rule must admit it. That is the pool separation this step used to lack.
     *
     * <p>
     * A run with no owner -- the vendor's own automation sync, and every Community run -- gets the weaker gate instead,
     * and deliberately NOT the full one: an empty owner opens both pools and admits every account's knowledge base, so
     * resolving with it would be the opposite answer rather than a weaker one. Such a run asks the question it can
     * answer, whether the target belongs to an account at all, and refuses it if so.
     */
    private Admission admit(Context context, long knowledgeBaseId) {
        ...
        return new Admission(
            KnowledgeBaseOptionsUtils.requireUnassignedKnowledgeBase(knowledgeBaseService, knowledgeBaseId),
            Optional.empty());
    }
