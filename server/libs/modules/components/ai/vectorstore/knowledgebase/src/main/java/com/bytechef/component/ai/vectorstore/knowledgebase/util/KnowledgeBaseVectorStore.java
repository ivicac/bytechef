...
public final class KnowledgeBaseVectorStore {
    ...
    private static class VectorStoreImpl implements VectorStore {
         ...
         *
         * <p>
         * Two gates rather than one, because they answer different questions and only one of them is answerable at any
         * given frame. With a context in hand the run's owner is knowable, so the full question is asked: may this
         * owner, in the pool {@code poolFor} allows them, reach this id. Without one the owner is not merely unknown
         * but UNKNOWABLE, and passing an empty owner would be the opposite answer rather than a weaker one -- an empty
         * owner opens both pools and admits every account's knowledge base. Such a frame asks the question it can
         * answer instead: is the target assigned to an account at all.
         * </p>
         */
