...
class KnowledgeBaseOptionsUtilsTest {
    ...
    void testAnOwnedRunNeverReadsTheAutomationPool() {
        ...
        when(knowledgeBaseService.getKnowledgeBases(PlatformType.EMBEDDED)).thenReturn(List.of());

        KnowledgeBaseOptionsUtils.knowledgeBases(knowledgeBaseService, owner);

        verify(knowledgeBaseService, never()).getKnowledgeBases(PlatformType.AUTOMATION);
    }
    ...
    void testAnUnownedRunMergesBothPools() {
        ...
        when(knowledgeBaseService.getKnowledgeBases(PlatformType.AUTOMATION))
            .thenReturn(List.of(automationKnowledgeBase));
        when(knowledgeBaseService.getKnowledgeBases(PlatformType.EMBEDDED))
    ...
    void testResolvingByIdNarrowsAnOwnedRunToTheEmbeddedPool() {
        ...
        verify(knowledgeBaseService).getKnowledgeBase(7L, List.of(PlatformType.EMBEDDED));
    }
    ...
    void testResolvingByIdAdmitsAVendorRunToBothPools() {
        ...
        verify(knowledgeBaseService)
            .getKnowledgeBase(7L, List.of(PlatformType.AUTOMATION, PlatformType.EMBEDDED));
    }
