...
public abstract class AbstractAiAgentChatAction {
            ...
            .tools(
                concatToolCallbacks(
                    getToolCallbacks(
                        ...
                        connectionParameters, toolExecutionListener, toolSimulations, chatModel, context),
                    chatMemoryResult)
                        .toArray());
    ...
    String resolveGatedToolResumeData(
        ...
        ToolCallback gatedToolCallback = clusterElementMap.getClusterElements(BaseToolFunction.TOOLS)
            .stream()
            .flatMap(
                clusterElement -> buildElementToolCallbacks(clusterElement, connectionParameters, context).stream())
            .filter(toolCallback -> {
    ...
    private List<ToolCallback> getToolCallbacks(
        ...
        for (ClusterElement clusterElement : toolClusterElements) {
            toolCallbacks.addAll(
                buildElementToolCallbacks(clusterElement, connectionParameters, context));
        }
    ...
    private List<ToolCallback> buildElementToolCallbacks(
        ClusterElement clusterElement, Map<String, ComponentConnection> connectionParameters, ActionContext context) {

        return clusterElementToolCallbacks.build(clusterElement, connectionParameters, context);
    }
