...
public class AssetFileDownloadAction {
    ...
    private ModifiableActionDefinition build() {
        return action("downloadAssetFile")
            .title("Download Asset File")
            .description("Download the binary content of an asset file as a workflow file entry.")
            .properties(
                integer(ASSET_FILE_ID)
                    .label("Asset File ID")
                    .description("Identifier of the asset file whose content to download.")
                    .required(true))
            .output(outputSchema(fileEntry().description("File entry containing the downloaded asset file content.")))
