...
class DataTableEmbeddedActionTest {
    ...
    @Test
    void testAnAccountsFindRecordsReadsThatAccountsOwnPhysicalTable() throws Exception {
        DataTableRef ownedDataTableRef =
            new DataTableRef("orders", ENVIRONMENT_ID, PlatformType.EMBEDDED, ACCOUNT_OWNER, ACCOUNT_OWNER);
    ...
    @Test
    void testAnotherAccountsFindRecordsReadsADifferentPhysicalTable() throws Exception {
        DataTableRef otherOwnedDataTableRef = new DataTableRef(
            "orders", ENVIRONMENT_ID, PlatformType.EMBEDDED, OTHER_ACCOUNT_OWNER, OTHER_ACCOUNT_OWNER);
        ...
        assertThat(otherOwnedDataTableRef.physicalName())
            .as("two accounts naming the same table must not share a physical table")
            .isNotEqualTo(
                new DataTableRef("orders", ENVIRONMENT_ID, PlatformType.EMBEDDED, ACCOUNT_OWNER, ACCOUNT_OWNER)
                    .physicalName());
    }
    ...
    @Test
    void testAnAccountsCreateRecordsWritesIntoThatAccountsOwnPhysicalTable() throws Exception {
        DataTableRef ownedDataTableRef =
            new DataTableRef("orders", ENVIRONMENT_ID, PlatformType.EMBEDDED, ACCOUNT_OWNER, ACCOUNT_OWNER);
    ...
    @Test
    void testAVendorStepNamingNoAccountWritesIntoTheSharedPhysicalTable() throws Exception {
        DataTableRef sharedDataTableRef = DataTableRef.shared("orders", ENVIRONMENT_ID, PlatformType.EMBEDDED);
        ...
        assertThat(sharedDataTableRef.physicalName())
            .as("shared tables are released data and must keep their unowned physical name")
    ...
    void testAnAccountsFindRecordsOnTheSharedTableScopesTheRowsToThatAccount() throws Exception {
        ...
        assertThat(dataTableRef.runOwner())
            .as("an account's read of the shared table must be scoped to that account's rows")
            .isEqualTo(ACCOUNT_OWNER);
        assertThat(dataTableRef.resourceOwner())
            .as("the shared table belongs to the vendor, so only the run owner separates the accounts")
            .isNull();
    }
                    ...
                    .thenAnswer(invocation -> {
                        ...
                        return Optional.of(
                            new DataTableResolution(
                                DATA_TABLE_ID,
                                new DataTableRef(
                                    "orders", ENVIRONMENT_ID, PlatformType.EMBEDDED, null, runOwner.orElse(null))));
                    });

        when(
            dataTableService.listTables(
                ArgumentMatchers.eq(ENVIRONMENT_ID), ArgumentMatchers.eq(PlatformType.EMBEDDED),
                ArgumentMatchers.<Optional<Owner>>any()))
                    .thenReturn(
                        List.of(
                            new DataTableInfo(
