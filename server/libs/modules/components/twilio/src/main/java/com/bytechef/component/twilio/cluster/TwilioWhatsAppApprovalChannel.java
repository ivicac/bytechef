...
/**
 * Approval channel that delivers the request as a WhatsApp message via Twilio. Field-less approvals get one-click
 ...
 * customer-service window — outside it Twilio requires a pre-approved template, so pair this channel with a fallback
 * for cold outreach.
 *
 * @author Ivica Cardic
 */
public class TwilioWhatsAppApprovalChannel {
    ...
    public static final ClusterElementDefinition<ApprovalChannelFunction> CLUSTER_ELEMENT_DEFINITION =
        ComponentDsl.<ApprovalChannelFunction>clusterElement("whatsApp")
            ...
            .properties(
                string(TO)
                    .label("To")
                    .description("The recipient channel address, e.g. whatsapp:+15554449999.")
                    .controlType(ControlType.PHONE)
                    .exampleValue("whatsapp:+15554449999")
                    .required(true),
                string(FROM)
                    .label("From")
                    .description("The sender's Twilio channel address, e.g. whatsapp:+15554449999.")
                    .controlType(ControlType.PHONE)
                    .exampleValue("whatsapp:+15554449999")
                    .required(true))
            .object(() -> TwilioWhatsAppApprovalChannel::perform);
    ...
    private static Object perform(
        ...
        Map<String, String> bodyMap = Map.of(
            FROM, normalizeWhatsAppAddress(inputParameters.getRequiredString(FROM)),
            TO, normalizeWhatsAppAddress(inputParameters.getRequiredString(TO)),
    ...
    private static String normalizeWhatsAppAddress(String address) {
        String trimmedAddress = address.trim();

        return trimmedAddress.startsWith("whatsapp:") ? trimmedAddress : "whatsapp:" + trimmedAddress;
    }
