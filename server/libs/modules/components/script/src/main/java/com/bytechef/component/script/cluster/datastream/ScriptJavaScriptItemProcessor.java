 ...
 * @author Ivica Cardic
 */
public class ScriptJavaScriptItemProcessor {

    public static ScriptClusterElementDefinition of(PolyglotEngine polyglotEngine) {
        return new ScriptClusterElementDefinition(
            clusterElement("javaScript")
                .title("JavaScript")
                .description("Transforms data stream items using custom JavaScript code.")
                .type(PROCESSOR)
                .properties(
                    string(SCRIPT)
    ...
    private ScriptJavaScriptItemProcessor() {
    }
}
