...
import static com.bytechef.component.commands.constant.CommandsConstants.PYTHON;
...
/**
 * The {@code python} action - its {@code commands} are lines handed to {@code python3} by default, the same default the
 * process runner falls back to for the {@code python} language when the {@code interpreter} property is left blank.
 *
 * @author Ivica Cardic
 */
public class CommandsPythonAction {
    ...
    public static CommandsActionDefinition of(TaskRunnerRegistry taskRunnerRegistry) {
        ...
        return new CommandsActionDefinition(
            action(PYTHON)
                .title("Python")
                .description("Runs Python commands in the selected task runner.")
                .properties(properties)
                .output(),
            PYTHON, taskRunnerRegistry);
    }

    private CommandsPythonAction() {
    }
}
