...
package com.bytechef.ee.automation.api_platform.handler;
...
import com.bytechef.ee.automation.api_platform.configuration.domain.ApiCollectionEndpoint;
import com.bytechef.ee.automation.api_platform.configuration.service.ApiCollectionEndpointService;
import com.bytechef.ee.automation.api_platform.configuration.service.ApiCollectionService;
 ...
 */
@RestController
@RequestMapping("${openapi.openAPIDefinition.base-path.automation:}/o/**")
public class ApiPlatformHandlerController {
    ...
    public Object handleGetMethod(final HttpServletRequest request) throws ServletException, IOException {
        return TenantUtils.callWithTenantId(TenantContext.getCurrentTenantId(), () -> {
            Map<String, List<String>> variables = Map.of();

            String requestURI = request.getRequestURI();

            requestURI = requestURI.replace("/api/automation/o", "");

            ApiCollectionEndpoint apiCollectionEndpoint = getApiCollectionEndpoint(requestURI);

            variables = PATH_MATCHER
                .extractUriTemplateVariables(apiCollectionEndpoint.getPath(), requestURI)
                .entrySet()
                .stream()
                .collect(
                    Collectors.toMap(
            ...
            WebhookTriggerFlags webhookTriggerFlags = new WebhookTriggerFlags(false, false, false, false);
    ...
    private ApiCollectionEndpoint getApiCollectionEndpoint(String requestURI) {
        for (ApiCollection apiCollection : apiCollectionService.getApiCollections()) {
            ...
            for (ApiCollectionEndpoint apiCollectionEndpoint : apiCollectionEndpoints) {
                if (PATH_MATCHER.match(apiCollectionEndpoint.getPath(), requestURI)) {
                    return apiCollectionEndpoint;
                }
            }
        }

        throw new IllegalArgumentException("No API Collection endpoint found for request uri: " + requestURI);
    }
