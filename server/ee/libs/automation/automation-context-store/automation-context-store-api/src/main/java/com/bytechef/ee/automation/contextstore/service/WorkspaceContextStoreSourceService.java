 ...
 * Phase 15 UI. The source row does not carry an environment column; a source takes its environment from its parent
 * store, and its {@code ProjectDeploymentWorkflow} lives on that environment's deployment. The parameter is reserved so
 * callers can pass the active environment without further GraphQL surface changes.
 *
 * @author Ivica Cardic
 * @version ee
 */
