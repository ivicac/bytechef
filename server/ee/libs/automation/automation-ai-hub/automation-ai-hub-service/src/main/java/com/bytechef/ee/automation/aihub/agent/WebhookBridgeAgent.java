...
import com.bytechef.ee.platform.aihub.agent.AgUiStreamBridge;
import com.bytechef.ee.platform.aihub.agent.BridgedFileEntry;
 ...
 * executor instead of the LLM. Built on top of {@link WebhookWorkflowExecutionFacade} so the existing HTTP/SSE
 * controller and this bridge share the same orchestration logic.
 *
 * <p>
 * Per-turn flow:
 * </p>
 ...
 * <li>Read {@link WebhookTriggerFlags#workflowSyncExecution()} from the facade. Sync triggers go through
 * {@link WebhookWorkflowExecutionFacade#executeSync}; streaming triggers through
 * {@link WebhookWorkflowExecutionFacade#executeStreaming} with an {@link AgUiStreamBridge}.</li>
...
public class WebhookBridgeAgent extends LocalAgent {
    ...
    protected void run(RunAgentInput input, AgentSubscriber subscriber) {
        ...
        try {
            hasStreamingTask = webhookFacade.hasStreamingTask(workflowExecutionId);
        } catch (RuntimeException exception) {
    ...
    private void executeSync(
        ...
        try {
            WebhookExecutionResult result = webhookFacade.executeSync(workflowExecutionId, webhookRequest);

            // Switch expression on the sealed interface so the compiler enforces exhaustiveness — adding a new
            // WebhookExecutionResult variant later forces a matching branch here. The previous if-instanceof
            // chain was non-exhaustive (no else) so a future variant would silently no-op, leaving the bridge
            // hung without firing onComplete or onError → the client UI would wait indefinitely (the "no
            // response" symptom users have reported).
            switch (result) {
                case WebhookExecutionResult.Disabled disabled -> {
                    // Belt-and-suspenders: the upfront isWorkflowDisabled check in run() catches this in the
                    // common case, but a workflow disabled BETWEEN the check and the executor call lands here.
                    // Surface the same friendly message regardless of the path that detected the state — the
                    // raw disabled.reason() is logged at DEBUG for ops visibility.
                    if (log.isDebugEnabled()) {
                        log.debug("Workflow chat disabled mid-turn: {}", disabled.reason());
                    }

                    bridge.onError(new IllegalStateException(disabledChatErrorMessage(task)));
                }
                case WebhookExecutionResult.Ok(@Nullable Object outputs) -> handleSyncOutputs(
                    outputs, bridge, subscriber, messageId, task, userMessageText);
            }
        } catch (RuntimeException exception) {
    ...
    private @Nullable WebhookTriggerFlags lookupTriggerFlagsOrEmitError(
        ...
        try {
            if (webhookFacade.isWorkflowDisabled(workflowExecutionId)) {
            ...
            return webhookFacade.getWebhookTriggerFlags(workflowExecutionId);
        } catch (RuntimeException exception) {
            log.warn(
