...
import com.bytechef.ee.platform.aihub.exception.ForbiddenException;
import com.bytechef.ee.platform.aihub.exception.NotFoundException;
...
public class AiHubTaskArtifactServiceImpl implements AiHubTaskArtifactService {
    ...
    public void record(
        ...
        if (taskOptional.isEmpty()) {
            ...
            log.warn(
                ...
                LogSanitizer.sanitizeForLog(threadId), userId, kind,
    ...
    public AiHubTaskArtifact recordReference(
        ...
        // Idempotency check by (taskId, kind, artifactId). The user re-attaching the same
        // file/workflow in the composer should hit the existing row rather than proliferate sidebar
        // entries. The unique tuple matches the artifact-list rendering — same row in the sidebar.
        ...
        try {
            ...
            log.warn(
                "Failed to serialize artifact metadata for kind={}, artifactId={} — recording artifact with "
                    + "null metadataJson.",
                kind, LogSanitizer.sanitizeForLog(artifactId), exception);
