...
/**
 * Unit tests for {@link AiHubTaskArtifactRecorderImpl}.
 *
 * @version ee
 *
 * @author Ivica Cardic
...
class AiHubTaskArtifactRecorderTest {
    ...
    private AiHubTaskArtifactRecorderImpl recorder;
    ...
    void setUp() {
        ...
        recorder =
            new AiHubTaskArtifactRecorderImpl(taskArtifactService, meterRegistryProvider);

        recorderLogger = (Logger) LoggerFactory.getLogger(AiHubTaskArtifactRecorderImpl.class);
    ...
    void testRecordDelegatesWhenUserIdProvided() {
        recorder.record(THREAD_ID, USER_ID, AiHubTaskArtifactKind.WORKFLOW_CREATED.name(), "wf-1",
            "Workflow A");

        verify(taskArtifactService).record(
            eq(THREAD_ID), eq(USER_ID), eq(AiHubTaskArtifactKind.WORKFLOW_CREATED),
            eq("wf-1"), eq("Workflow A"), any());
    }

    @Test
    void testRecordSkipsAndWarnsWhenUserIdMissing() {
        recorder.record(THREAD_ID, null, AiHubTaskArtifactKind.WORKFLOW_CREATED.name(), "wf-1",
            "Workflow A");

        verify(taskArtifactService, never()).record(
        ...
        assertThat(formattedMessage).contains(AiHubTaskArtifactKind.WORKFLOW_CREATED.name());
    ...
    void testRecordIncrementsMissingUserIdCounterWhenUserIdMissing() {
        ...
        recorder.record(THREAD_ID, null, AiHubTaskArtifactKind.WORKFLOW_CREATED.name(), "wf-2",
            "Workflow B");
