...
class ListConnectionsForComponentToolCallbackTest {
    ...
    void testListsConnectionsForComponent() throws Exception {
        ...
        PropertyOptionsResolver resolver = new PropertyOptionsResolver(userService, mock(AuthorityService.class));

        ListConnectionsForComponentToolCallback callback = new ListConnectionsForComponentToolCallback(
            connectionDefinitionService, workspaceConnectionFacade, resolver, mock(AiHubToolAttachMetrics.class),
            jsonMapper);
    ...
    @Test
    void testReturnsEmptyEnvelopeWhenConnectionDefinitionLookupFails() throws Exception {
        ConnectionDefinitionService connectionDefinitionService = mock(ConnectionDefinitionService.class);

        when(connectionDefinitionService.getConnectionDefinition("nonexistent", 1))
            .thenThrow(new RuntimeException("unknown component"));

        PropertyOptionsResolver resolver = new PropertyOptionsResolver(
            mock(UserService.class), mock(AuthorityService.class));

        ListConnectionsForComponentToolCallback callback = new ListConnectionsForComponentToolCallback(
            connectionDefinitionService, mock(WorkspaceConnectionFacade.class), resolver,
            mock(AiHubToolAttachMetrics.class), jsonMapper);

        ToolContext toolContext = new ToolContext(
            new AiHubToolInvocationContext(1L, 10L, (short) 0, "x", 0L, "thread-1").toToolContext());

        String result = callback.call("{\"componentName\":\"nonexistent\"}", toolContext);
    ...
    void testReturnsToolErrorWhenWorkspaceContextMissing() throws Exception {
        ...
        ListConnectionsForComponentToolCallback callback = new ListConnectionsForComponentToolCallback(
            mock(ConnectionDefinitionService.class), mock(WorkspaceConnectionFacade.class), resolver,
            mock(AiHubToolAttachMetrics.class), jsonMapper);
    ...
    void testRejectsBlankComponentName() throws Exception {
        ...
        assertThat(node.has("error")).isTrue();
        assertThat(node.get("error")
            .asText()).contains("componentName is required");
    }
