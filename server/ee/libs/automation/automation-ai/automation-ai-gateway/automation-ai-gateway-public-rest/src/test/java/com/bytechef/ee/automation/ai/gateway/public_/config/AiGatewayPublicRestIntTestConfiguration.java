 ...
 * @version ee
 */
// Narrow the scan to just the OTLP controller + its dependency chain. The chat/embedding/routing controllers pull in
// AiGatewayFacade which in turn drags in the whole provider/compression/evaluation/rag stack; those are orthogonal to
// OTLP ingest and would require mocking several more beans. Restrict the web.rest scan to AiGatewayOtlpController and
// its exception-handler advice via an include-filter.
@ComponentScan(
    basePackages = {
