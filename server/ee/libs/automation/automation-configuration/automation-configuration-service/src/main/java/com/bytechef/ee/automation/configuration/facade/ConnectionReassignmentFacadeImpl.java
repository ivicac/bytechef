...
public class ConnectionReassignmentFacadeImpl implements ConnectionReassignmentFacade {
    ...
    public void reassignAllConnections(long workspaceId, String userLogin, String newOwnerLogin) {
        ...
        // reassemble which rows belong to the same bulk reassignment. The correlation ID is
     ...
     *
     * <p>
     * The supplied {@code correlationId} groups every per-row event from the same bulk invocation.
     *
     * <p>
    ...
    private void publishReassignmentAudit(
        ...
        try {
            ...
            log.error("Failed to publish CONNECTION_REASSIGNED audit for id={}", connectionId, exception);
        }
    }
