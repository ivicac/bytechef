...
class PermissionServiceAgentVisibilityTest {
    ...
    /**
     * The hole a resource-role disjunct on an agent-keyed sharing gate closes, pinned as the two halves it is made of
     * rather than as its conclusion. Ana is a workspace ADMIN here and not a tenant admin, so the scope check that
     ...
     * allows her, and the gate's {@code ||} is what turns that into a re-share. Drop the disjunct from such a gate and
    ...
    private static PermissionServiceImpl permissionService(Set<Long> grantedProjectIds, WorkspaceRole workspaceRole) {
        ...
        AiAgent agent = agent(AGENT_ID);
        AiAgent workspaceAgent = agent(WORKSPACE_AGENT_ID);

        when(aiAgentRepository.findById(AGENT_ID)).thenReturn(Optional.of(agent));
        when(aiAgentRepository.findById(WORKSPACE_AGENT_ID)).thenReturn(Optional.of(workspaceAgent));
        when(aiAgentRepository.findProjectIdByProjectWorkflowUuid(agent.getProjectWorkflowUuid()))
            .thenReturn(Optional.of(PROJECT_ID));
        when(aiAgentRepository.findProjectIdByProjectWorkflowUuid(workspaceAgent.getProjectWorkflowUuid()))
            .thenReturn(Optional.of(WORKSPACE_PROJECT_ID));
