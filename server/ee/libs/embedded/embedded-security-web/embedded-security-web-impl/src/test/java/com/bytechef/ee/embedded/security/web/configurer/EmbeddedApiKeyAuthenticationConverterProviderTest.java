 ...
 * it ever produces an {@code Authentication} for {@link EmbeddedApiKeyAuthenticationProvider} to authenticate -- so the
 * provider's get-or-create ({@code ConnectedUserService#fetchConnectedUser}/ {@code #createConnectedUser}) is never
 * reached and no phantom {@code ConnectedUser} row is created. The
...
class EmbeddedApiKeyAuthenticationConverterProviderTest {
    ...
    void setUp() {
        ...
        // Constructed to prove the shape of the real collaborator, even though this test never calls authenticate()
        // on it -- the whole point is that the filter chain must never reach it for this request.
