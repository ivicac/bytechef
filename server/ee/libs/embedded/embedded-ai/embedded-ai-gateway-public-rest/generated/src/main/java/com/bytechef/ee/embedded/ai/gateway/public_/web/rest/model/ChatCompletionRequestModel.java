...
/**
 * A chat completion request. Field-for-field the same OpenAI-compatible shape the automation gateway accepts; the two are kept in step by hand, not by a shared generated model.
 */

@Schema(name = "ChatCompletionRequest", description = "A chat completion request. Field-for-field the same OpenAI-compatible shape the automation gateway accepts; the two are kept in step by hand, not by a shared generated model.")
...
public class ChatCompletionRequestModel {
  ...
  /**
   * The messages to generate chat completions for. Must not be empty.
   * @return messages
   */
  @NotNull @Valid 
  @Schema(name = "messages", description = "The messages to generate chat completions for. Must not be empty.", requiredMode = Schema.RequiredMode.REQUIRED)
  ...
  /**
   * Whether the caller intends to stream. It does NOT select the response shape -- the Accept header does. Sending true without `Accept: text/event-stream` is a 400. Omitted is treated as false.
   * @return stream
   */
  
  @Schema(name = "stream", description = "Whether the caller intends to stream. It does NOT select the response shape -- the Accept header does. Sending true without `Accept: text/event-stream` is a 400. Omitted is treated as false.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
  @JsonProperty("stream")
  ...
  /**
   * The name of the routing policy to use. When omitted, the connected user's default routing policy applies, falling back to the embedded default for the environment.
   * @return routingPolicy
   */
  
  @Schema(name = "routing_policy", description = "The name of the routing policy to use. When omitted, the connected user's default routing policy applies, falling back to the embedded default for the environment.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
  ...
  /**
   * Controls which tool is called by the model. Either a bare string (\"auto\", \"none\", \"required\") or an object of the form {\"type\": \"function\", \"function\": {\"name\": \"...\"}}. Any other shape is a 400. Typed as object rather than a oneOf because openapi-generator renders a oneOf as an empty marker interface that nothing can implement; the generated field is a raw Object, which accepts both shapes.
   * @return toolChoice
   */
  
  @Schema(name = "tool_choice", description = "Controls which tool is called by the model. Either a bare string (\"auto\", \"none\", \"required\") or an object of the form {\"type\": \"function\", \"function\": {\"name\": \"...\"}}. Any other shape is a 400. Typed as object rather than a oneOf because openapi-generator renders a oneOf as an empty marker interface that nothing can implement; the generated field is a raw Object, which accepts both shapes.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
