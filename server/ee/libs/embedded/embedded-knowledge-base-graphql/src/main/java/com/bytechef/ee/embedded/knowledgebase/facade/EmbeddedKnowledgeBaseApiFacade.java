...
public interface EmbeddedKnowledgeBaseApiFacade {
    ...
    /**
     * Creates a new knowledge base in the embedded pool, either the vendor's own or one connected account's.
     *
     * <p>
     * An account's knowledge base is how a per-account override is created: the vendor's workflow keeps naming
     * {@code docs}, and a run belonging to that account resolves its copy instead of the shared one.
     * {@link #assignKnowledgeBaseOwner} cannot produce this without first creating a shared knowledge base of that name
     * -- which is exactly the name the account's copy would collide with.
     *
     ...
     * @param ownerId          the connected user the knowledge base belongs to, or null to create it shared with every
     *                         account
