...
/**
 * Per-task binding of a ByteChef component to an optional connection. Mirrors the {@code McpComponent} shape but scopes
 * lifetime to a single AI Hub task.
 *
 * <p>
 * One row per (task, component, version, connection, environment) tuple — the unique constraint
 * {@code uk_ccc_conv_comp_conn_env} makes attach idempotent: re-issuing the chat command "add Slack to this task" hits
 ...
 * matches the surrounding ai-hub task domain (see {@link AiHubTaskAssetFile}); McpComponent's AggregateReference style
 ...
 * @author Ivica Cardic
 */
@Table("ai_hub_task_component")
public class AiHubTaskComponent {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof AiHubTaskComponent that)) {
            return false;
        }
