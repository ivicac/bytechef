 ...
 * retry-template construction happen only once. Per-call state (prompt, size, style) still builds fresh
 * {@link OpenAiImageOptions}. The response format is always {@code b64_json} so the caller receives raw bytes rather
 * than a time-limited URL.
 *
 * @version ee
 *
 * @author Ivica Cardic
