...
/**
 * Loads tool catalogs into a {@link VectorToolSearcher}. Two indexing modes exist:
 *
 * <ul>
 ...
 * how we tell the {@link VectorToolSearcher} "search this subset, not the workspace catalog" without re-architecting
 * the underlying pgvector store.
 * </p>
 *
 * <p>
 * <b>Re-index semantics:</b> each populate call clears its target session's in-memory document-id map first via
 * {@link VectorToolSearcher#clearIndex(String)}, then re-issues an {@code indexTool} call per entry. Two implications:
 * </p>
 * <ul>
...
public class ToolSearchCatalogFeeder {
    ...
    public ToolSearchCatalogFeeder(
        ClusterElementDefinitionService clusterElementDefinitionService, VectorToolSearcher vectorToolSearcher,
