...
/**
 * Spring AI {@link ToolCallback} the LLM uses to attach a configured tool to the current task. Combines the two facade
 * calls (attach component + add tool) in one shot because the LLM never knows the internal {@code taskComponentId} — it
 ...
 * <li>LLM (after a connection-resolution step): {@code attachTaskTool({componentName: "slack", actionName:
 * "sendMessage", parameters: {channel: "#engineering"}, connectionId: 42})}</li>
 * <li>This callback: resolves task_id from thread, attaches component (idempotent), upserts tool with the supplied
 ...
 * @author Ivica Cardic
 */
public class AttachTaskToolToolCallback implements ToolCallback {

    static final String TOOL_NAME = "attachTaskTool";

    private static final String DESCRIPTION = """
        Attach a configured tool to the current task so the assistant can invoke it directly in
        ...
        {taskToolId, taskComponentId, attachedToolName, replacedExistingTool}.""";
    ...
    public AttachTaskToolToolCallback(
        AiHubTaskService taskService,
        AiHubTaskToolFacade taskToolFacade, ConnectionService connectionService, AiHubToolAttachMetrics metrics) {
    ...
    public String call(String toolInput, @Nullable ToolContext toolContext) {
        try {
            AttachTaskToolInput input = jsonMapper.readValue(toolInput, AttachTaskToolInput.class);
            ...
            // call creates a DUPLICATE row. We detect the (task, component, version, env) match ignoring
            ...
            Optional<Long> existingComponentBindingId = taskToolFacade.findTaskComponentIdIgnoringConnection(
                task.get()
                    .getId(),
                input.componentName(), componentVersion, environment);

            long taskComponentId;

            if (existingComponentBindingId.isPresent()) {
                taskComponentId = existingComponentBindingId.get();

                taskToolFacade.setComponentConnection(taskComponentId, connectionId);

                metrics.recordAttach("rebound_connection");
            } else {
                taskComponentId = taskToolFacade.attachComponent(
                    task.get()
                        .getId(),
            ...
            long taskToolId = taskToolFacade.addTool(
                taskComponentId, input.actionName(), parameters);

            return jsonMapper.writeValueAsString(new AttachTaskToolOutput(
                taskToolId, taskComponentId,
            ...
            return ToolErrors.runtimeFailure(
                jsonMapper, AttachTaskToolToolCallback.class, TOOL_NAME, exception);
        }
    }

    /**
     * Returns a user-facing error message when the connection's environment does not match the task's, or {@code null}
     ...
     */
    private @Nullable String checkConnectionEnvironment(long connectionId, int taskEnvironment) {
        ...
        if (connection.getEnvironmentId() != taskEnvironment) {
            return "Connection " + connectionId + " belongs to environment " + connection.getEnvironmentId()
                + " but this chat is rooted in environment " + taskEnvironment
    ...
    public record AttachTaskToolInput(
        ...
        public AttachTaskToolInput {
            parameters = parameters == null ? null : Map.copyOf(parameters);
        }
    }

    public record AttachTaskToolOutput(
        long taskToolId, long taskComponentId, String attachedToolName,
