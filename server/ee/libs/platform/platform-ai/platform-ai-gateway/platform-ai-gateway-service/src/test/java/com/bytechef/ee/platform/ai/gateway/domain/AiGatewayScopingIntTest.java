...
/**
 * Asserts the connected-user scoping added to {@code ai_gateway_provider} against the real database rather than against
 * the changelog text: the check constraint {@code ck_ai_gateway_provider_workspace_connected_user_not_both} and the
 * provider partial unique index {@code uk_ai_gateway_provider_connected_user_id_type} (composite: a connected user may
 * legitimately hold one BYOK provider per provider type) — both added by
 * {@code platform/ai/gateway/00000000000001_ai_gateway_init.xml} (changelog is branch-only/unreleased, so both live in
 * the init changeset itself rather than a follow-up one). Also asserts the connected-user settings unique constraint
 * {@code uk_ai_gateway_connected_user_settings_connected_user_id}: a connected user may hold at most one settings row
 * (assigned plan and spend cap).
 *
 * <p>
...
class AiGatewayScopingIntTest {
    ...
    /**
     * A connected user has at most one settings row (assigned plan and spend cap); a second row for the same connected
     * user is rejected by {@code uk_ai_gateway_connected_user_settings_connected_user_id}.
     */
    @Test
    void testSecondSettingsRowForSameConnectedUserIsRejectedByUniqueConstraint() {
        insertConnectedUserSettings(300L);

        assertThatThrownBy(() -> insertConnectedUserSettings(300L))
            .isInstanceOf(DataIntegrityViolationException.class);
    }

    private void insertConnectedUserSettings(long connectedUserId) {
        jdbcTemplate.update(
            "INSERT INTO ai_gateway_connected_user_settings (connected_user_id, created_date, last_modified_date, " +
