...
/**
 * Pins the precedence rules for {@link CopilotChatClientResolver#resolve}. Copilot has no per-agent override layer, so
 * the precedence is just user-selected (chat-toolbar picker) or workspace default — the resolver returns {@code null}
 * for the workspace-default case so the caller (the Copilot agent) falls back to its builder-time
 * {@code @Primary ChatModel}.
 *
 * @version ee
 *
 * @author Ivica Cardic
