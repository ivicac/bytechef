...
public class AiHubChatGraphQlController {
     ...
     *
     * <p>
     * Returns a {@code Map} keyed by the parent, not a positional {@code List}. spring-graphql wraps a returned
     * {@code List} in {@code Flux.fromIterable}, and Reactive Streams forbids {@code null} elements, so a single
     * unresolvable login would fail the whole batch with "The iterator returned a null value". In the map form a
     * missing key simply resolves to {@code null}, which the nullable {@code ownerName} schema field expects: a deleted
     * user must not blank the row. Equal keys resolve to the same entry, which is correct because equal parents carry
     * the same user id.
     * </p>
     */
    @BatchMapping(typeName = "AiHubChat", field = "ownerName")
    public Map<AiHubChat, String> chatOwnerName(List<AiHubChat> chats) {
        Map<Long, String> loginByUserId = resolveLoginsByUserId(
            chats.stream()
                .map(AiHubChat::getUserId));

        return mapResolvedLogins(chats, AiHubChat::getUserId, loginByUserId);
    }
     ...
     * turn attribution is unreliable, see {@code AiHubChatServiceImpl.loadMessages}, plus every assistant row — is left
     * out of the returned map, so it resolves to {@code null} and contributes no lookup.
     */
    @BatchMapping(typeName = "AiHubChatMessage", field = "authorName")
    public Map<AiHubChatMessage, String> messageAuthorName(List<AiHubChatMessage> messages) {
        Map<Long, String> loginByUserId = resolveLoginsByUserId(
            messages.stream()
                .map(AiHubChatMessage::authorUserId));

        return mapResolvedLogins(messages, AiHubChatMessage::authorUserId, loginByUserId);
    }

    /**
     * Keys each parent to its resolved login, omitting parents whose user id is null or unresolved. Omitting rather
     * than storing {@code null} is what the batch contract requires: DataLoader reads a missing key as {@code null}.
     */
    private static <T> Map<T, String> mapResolvedLogins(
        List<T> parents, Function<T, @Nullable Long> userIdFunction, Map<Long, String> loginByUserId) {

        Map<T, String> loginByParent = new HashMap<>();

        for (T parent : parents) {
            Long userId = userIdFunction.apply(parent);

            if (userId == null) {
                continue;
            }

            String login = loginByUserId.get(userId);

            if (login != null) {
                loginByParent.put(parent, login);
            }
        }

        return loginByParent;
    }
