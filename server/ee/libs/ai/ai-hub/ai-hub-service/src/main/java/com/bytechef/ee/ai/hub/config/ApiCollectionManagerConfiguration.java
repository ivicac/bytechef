 ...
 * the ai_hub BUILD agent bean method (via {@link #createApiCollectionManagerToolCallback}) so that it is registered
 * only on that agent — the ASK agent keeps its read-only {@code listApiCollections} flat registration.
 * </p>
 *
 * @version ee
 *
...
public class ApiCollectionManagerConfiguration {
    ...
    static ManagerSubAgentToolCallback createApiCollectionManagerToolCallback(
        ChatClient apiCollectionManagerChatClient) {

        return new ManagerSubAgentToolCallback(
            AiHubAgentType.API_COLLECTION_MANAGER, apiCollectionManagerChatClient, TOOL_DESCRIPTION);
    }
