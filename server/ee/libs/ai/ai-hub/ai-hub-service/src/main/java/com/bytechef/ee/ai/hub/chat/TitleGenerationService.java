...
/**
 * Generates a short human-readable title for a chat by sending a condensed message excerpt to the configured
 * {@link ChatModel}.
 *
 * <p>
...
public class TitleGenerationService {
    ...
    /**
     * Generates a title from the first few messages in the chat.
     *
    ...
    public String generateTitle(List<AiHubChatMessage> messages, @Nullable Long workspaceId) {
        ...
        try {
            ...
            // model is unhealthy. The chat itself remains usable; the controller still returns the
    ...
    private List<AiHubChatMessage> selectMessages(List<AiHubChatMessage> messages) {
        List<AiHubChatMessage> selected = new ArrayList<>();

        int userCount = 0;
        int assistantCount = 0;

        for (AiHubChatMessage message : messages) {
