...
import type {VoiceEventI, VoiceSessionStatusType} from './BrowserVoiceSession';
import type {RealtimeVoiceAdapter} from '@assistant-ui/react';

/**
 * Ported from `client/src/shared/lib/voice/ByteChefRealtimeVoiceAdapter.test.ts`. The only real difference is
 * the activity holder: the platform copy drives a Zustand store through `voiceActivityStore.getState().X()`;
 * this copy drives the SDK's hand-rolled holder (`src/lib/useVoiceActivityStore.ts`) through `voiceActivityStore.X()`
 * directly — `getState()` here returns a plain `{activity, endReason}` snapshot with no actions attached.
 */
...
describe('ByteChefRealtimeVoiceAdapter', () => {
    ...
    it('resets a previous session end reason on connect, and records the reason on session_end', async () => {
        voiceActivityStore.setEndReason('previous_reason');
