...
/**
 * Ported from `client/src/shared/lib/voice/VoiceModeLayout.test.tsx`. The only difference is the activity
 * holder: the platform copy resets state via `useVoiceActivityStore.setState({...})` (Zustand); this copy
 * drives the SDK's hand-rolled holder (`src/lib/useVoiceActivityStore.ts`) via `voiceActivityStore.setState({...})`
 * directly — same call shape, no Zustand.
 */
describe('VoiceModeLayout', () => {
    beforeEach(() => {
        voiceActivityStore.setState({activity: 'idle', endReason: null});
    });
    ...
    it('shows the tool being looked up', () => {
        voiceActivityStore.setState({activity: {tool: 'lookupOrder'}, endReason: null});
    ...
    it('shows "Reconnecting…" while the session is reconnecting', () => {
        voiceActivityStore.setState({activity: 'reconnecting', endReason: null});
    ...
    it('shows the end reason when the session ended', () => {
        voiceActivityStore.setState({activity: 'idle', endReason: 'silence_timeout'});
    ...
    it('does not let a stale timer from a previous end reason hide a newer one early', () => {
        vi.useFakeTimers();
        voiceActivityStore.setState({activity: 'idle', endReason: 'first_reason'});
        ...
        act(() => {
            voiceActivityStore.setState({endReason: 'second_reason'});
        });
