...
The schema needs no data step. The *behaviour* does: a bridged automation workflow now resolves the
EMBEDDED pool, so one that reached an automation data table or knowledge base by name stops finding it
after the upgrade — and this default is why, since it lands every existing row on AUTOMATION.
See `2026-09-01-embedded-automation-resource-pools-migration-note.md`.
