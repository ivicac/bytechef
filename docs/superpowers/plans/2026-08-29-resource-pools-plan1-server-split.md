...
`unownedOnly()` is needed here, so add it to `RowOwnerFilter` in this task rather than in Task 5:

```java
    /**
     * Rows belonging to nobody. What a vendor run sees in an EMBEDDED table when it has not named an account --
     * unrestricted() would hand it every account's rows -- and, on a write, the explicit "this row is shared"
     * marker.
     */
...
Match the existing record/constructor shape; the third component is the "restrict to NULL owner" flag the SQL
builder reads in Task 5.

`DataTablePoolIsolationIntTest` also autowires the row service:
...
class DataTableRowPoolScopingIntTest {
    ...
    void testAnUnownedRunSeesOnlyUnownedRowsOfAnEmbeddedTable() {
        ...
        dataTableRowService.insertRow(
            "scoped", Map.of("title", "alice"), ENVIRONMENT_ID, PlatformType.EMBEDDED,
            RowOwnerFilter.from(Optional.of(Owner.connectedUser(1055L))));
        dataTableRowService.insertRow(
            "scoped", Map.of("title", "shared"), ENVIRONMENT_ID, PlatformType.EMBEDDED,
            RowOwnerFilter.unownedOnly());

        List<DataTableRow> rows = dataTableRowService.listRows(
            "scoped", ENVIRONMENT_ID, PlatformType.EMBEDDED, RowOwnerFilter.unownedOnly());
    ...
    void testNamingAnAccountReachesThatAccountsRows() {
        ...
        dataTableRowService.insertRow(
            "named", Map.of("title", "alice"), ENVIRONMENT_ID, PlatformType.EMBEDDED,
            RowOwnerFilter.from(Optional.of(Owner.connectedUser(1055L))));

        List<DataTableRow> rows = dataTableRowService.listRows(
            "named", ENVIRONMENT_ID, PlatformType.EMBEDDED,
            RowOwnerFilter.from(Optional.of(Owner.connectedUser(1055L))));
...
- [ ] **Step 4: Confirm `unownedOnly` is present**

`RowOwnerFilter.unownedOnly()` was added in Task 2, which needed it to write an embedded row. Nothing to add
here — `from(Optional.empty())` stays `unrestricted()`, which is correct for the AUTOMATION pool, whose rows
have no owners, and for Community Edition.
