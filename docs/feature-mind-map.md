...
mindmap
  root((ByteChef))
    ...
    AI Hub
      ...
      Conversation Kinds and Routing
      Personal Agents
      Tasks Sidebar, Rail and Home Panel
      Composer
      User Connectors and External MCP Servers
      Task Tools Autonomous Tool Attachment
      ...
      Artifact Generation
      Task Artifacts and Audit Trail
      Progress Narration
      Workflow Chats
...
mindmap
  root((AI Hub))
    AI Hub Chat Surface
      ...
      Message truncation / rewind truncateAiHubTaskMessages, us…
    ...
    Conversation Kinds and Routing
      STANDARD tasks — NanoID-style client threadId, AG-UI runt…
      WORKFLOW_CHAT tasks — UUID threadId, carry workflow_execu…
      PERSONAL_AGENT tasks — UUID threadId, carry ai_hub_person…
      Always-new-conversation semantics each workflow/agent row…
      Personal-agent system-prompt overlay applyPersonalAgentOv…
      Enum ordinal stability pinned by EnumOrdinalStabilityTest…
    Personal Agents
      Agent list page and cards AiHubPersonalAgents, AiHubPerso…
      Create/edit form AiHubPersonalAgentForm with save validat…
      Resources card attaching files/workflows/data tables/know…
      Per-agent tool configuration dialog AiHubPersonalAgentToo…
      Delete confirmation dialog AiHubPersonalAgentDeleteDialog
      Schedules tab recurring agent runs AiHubPersonalAgentSche…
      Scheduled runs fire headlessly AgentScheduleFiredEventLis…
      Workspace-shared agents WorkspaceAiHubPersonalAgent / Wor…
      GraphQL aiHubPersonalAgents/aiHubPersonalAgent queries, s…
    Tasks Sidebar, Rail and Home Panel
      Tasks sidebar with task list and switching AiHubTasksSide…
      Collapse button + collapsed rail AiHubTasksSidebarCollaps…
      Home panel AI-provider setup nudge links to /automation/s…
      Live workflow-chat label updates useLiveWorkflowLabel
      Task deletion deleteAiHubTask, workflow-chat badge in pan…
      Task status filtering AiHubTaskStatus, default ACTIVE
      GraphQL aiHubTasks, aiHubTaskMessages, createAiHubTask, c…
    Composer
      ...
      Referenced resources recorded as task artifacts recordRef…
    User Connectors and External MCP Servers
    ...
    Task Tools Autonomous Tool Attachment
      Attached tool chips in UI tools/TaskToolChips
      Tool config dialogs TaskToolDialog, ToolConfigDialog and…
      ...
      GraphQL bindings surface aiHubTaskTools, updateAiHubTaskT…
    ...
    Three-Tier Tool Architecture
      ...
      LazyToolCallingManager + AiHubTaskBindingToolCallbackReso…
    ...
    Specialist Subagents AiHubAgentType
      ...
      Manager subagents via ManagerSubAgentToolCallback mcp_man…
      Copilot specialist delegates registerCopilotSubAgentToolC…
      Manager subagents also contributed to the management MCP…
    ...
    Artifact Generation
      ...
      Asset-file tools createAssetFile, createBinaryAssetFile,…
      Asset files attributed to the producing surface via the a…
    Task Artifacts and Audit Trail
      Artifact recorders AiHubTaskArtifactRecorderImpl, Workflo…
      Per-task artifact listing aiHubTaskArtifactsByAiHubTask
      ...
      Task asset-file relationships AiHubTaskAssetFile / AiHubT…
    Progress Narration
    ...
    Workflow Chats
      ...
      Idempotent task creation per workflow pick manual titles…
      Bulk archive of workflow-chat tasks
...
- **AI Hub Chat Surface** [EE] — Full-page agent chat at /automation/ai-hub (client/src/pages/automation/ai-hub/AiHub.tsx, AiHubPanel.tsx) backed by the EE ai-hub modules in server/ee/libs/ai/ai-hub (ai-hub-api/-service/-graphql/-rest). Users converse with an LLM agent that can inspect and mutate workspace resources.
    ...
    - Per-task LLM model picker with last-used-model persistence (ModelPicker in AiHubPanel; personal-agent override > task override > workspace default)
    - Environment selector in panel header
    - Task title auto-generation (TitleGenerationService, generateAiHubTaskTitle mutation, kicked off around message-count >= 6; 'New Task' placeholder until then)
    - Message truncation / rewind (truncateAiHubTaskMessages, useTruncateTaskMessages)
...
- **ASK / BUILD Agent Modes** [EE] — Two agent modes selected client-side (useAiHubStore MODE.ASK/MODE.BUILD, default BUILD) and dispatched server-side via the append-only Mode enum crossing the wire in state.mode. ASK is read-only answer-building; BUILD is state-mutating construction with a much larger tool set.
    ...
    - Converter specialist is BUILD-only
- **Conversation Kinds and Routing** [EE] — AiHubTaskKind (INT-ordinal enum in ai-hub-api/task) discriminates ai_hub_task rows: STANDARD (LLM agent via AG-UI runAgent), WORKFLOW_CHAT (bound to a workflow execution, routed through WebhookBridgeAgent webhook endpoints instead of the LLM), PERSONAL_AGENT (LLM agent plus a per-agent instructions overlay). AiHubRoutingAgent looks up the task by threadId and dispatches.
    - STANDARD tasks — NanoID-style client threadId, AG-UI runtime
    - WORKFLOW_CHAT tasks — UUID threadId, carry workflow_execution_id + project_deployment_id for sidebar grouping
    - PERSONAL_AGENT tasks — UUID threadId, carry ai_hub_personal_agent_id
    - Always-new-conversation semantics: each workflow/agent row click creates a fresh task with its own threadId; past conversations stay in the list
    - Personal-agent system-prompt overlay (applyPersonalAgentOverlay injects personalAgentInstructions + personalAgentTitle state keys; appendPersonalAgentContext renders an advisory Context block)
    - Enum ordinal stability pinned by EnumOrdinalStabilityTest (append-only)
- **Personal Agents** [EE] — User-created per-workspace agent definitions (name, description, instructions, per-agent model override) chatted with as PERSONAL_AGENT tasks. Client CRUD UI under personal-agents/; server entities/services in ai-hub-api/-service personalagent packages with GraphQL controller.
    - Agent list page and cards (AiHubPersonalAgents, AiHubPersonalAgentsList)
    - Create/edit form (AiHubPersonalAgentForm) with save validation (PersonalAgentSaveValidator)
    - Resources card attaching files/workflows/data tables/knowledge bases to an agent (AiHubPersonalAgentResourcesCard, AiHubPersonalAgentResource + AiHubPersonalAgentResourceKind, removeAiHubPersonalAgentResource)
    - Per-agent tool configuration dialog (AiHubPersonalAgentToolConfigDialog, AiHubPersonalAgentTool, removeAiHubPersonalAgentTool)
    - Delete confirmation dialog (AiHubPersonalAgentDeleteDialog)
    - Schedules tab: recurring agent runs (AiHubPersonalAgentScheduleTab, frequency fields, ScheduleFrequencyKind / ScheduleLifecycleKind, ScheduleCronNormalizer)
    - Scheduled runs fire headlessly: AgentScheduleFiredEventListener creates a task row then AiHubScheduledChatDispatcher posts the turn without an HTTP client listening (output persisted to session memory)
    - Workspace-shared agents (WorkspaceAiHubPersonalAgent / WorkspaceAiHubPersonalAgentService)
    - GraphQL: aiHubPersonalAgents/aiHubPersonalAgent queries, save/delete mutations, schedule mutations
- **Tasks Sidebar, Rail and Home Panel** [EE] — Conversation management chrome: a collapsible tasks sidebar listing ACTIVE tasks grouped by kind, a slim icon rail when collapsed, and a home/landing panel for starting new conversations.
    - Tasks sidebar with task list and switching (AiHubTasksSidebar, useTasks, useSwitchTask, useAiHubTasksStore)
    - Collapse button + collapsed rail (AiHubTasksSidebarCollapseButton, AiHubTasksSidebarRail)
    - Home panel: AI-provider setup nudge (links to /automation/settings/ai-providers), draft LLM selection, personal-agent picker, chat-workflow picker (AiHubHomePanel)
    - Live workflow-chat label updates (useLiveWorkflowLabel)
    - Task deletion (deleteAiHubTask), workflow-chat badge in panel header
    - Task status filtering (AiHubTaskStatus, default ACTIVE)
    - GraphQL: aiHubTasks, aiHubTaskMessages, createAiHubTask, createWorkflowChatAiHubTask (idempotent re-pick returns existing task), bulkArchiveWorkflowChatAiHubTasks
- **Composer** [EE] — The message input area (composer/AiHubComposer, AiHubChatComposer) with attachments, resource references, connector and skills menus.
    ...
    - Referenced resources recorded as task artifacts (recordReferencedAiHubTaskArtifact mutation, useRecordReferencedArtifacts; idempotent, deletable via deleteAiHubTaskArtifact for reference kinds only)
...
- **Task Tools (Autonomous Tool Attachment)** [EE] — The agent can attach component actions/triggers to the current task at runtime as callable tools (AttachTaskToolToolCallback / RemoveTaskToolToolCallback, AiHubTaskToolFacade), with client-side chips and config dialogs for the resulting bindings.
    - Attached tool chips in UI (tools/TaskToolChips)
    - Tool config dialogs (TaskToolDialog, ToolConfigDialog) and cache (useTaskToolsCache)
    - State-visibility read tools so the LLM avoids duplicate attaches (listTaskTools, listConnectionsForComponent)
    ...
    - GraphQL bindings surface: aiHubTaskTools, updateAiHubTaskToolParameters, removeAiHubTaskTool, detachAiHubTaskComponent
...
- **Three-Tier Tool Architecture** [EE] — Tools reach the agents through (1) a pinned static list kept callable every iteration by PinnedToolSearchToolCallingAdvisor, (2) a searchable pgvector tool catalog surfaced via searchTool, and (3) one-shot specialist subagent delegate tools. Wired in AiHubConfiguration.
    - ASK pinned tools: research delegate, openResourceTab, openWorkflowChatTab, getAssetFileContent, listAssetFiles, askUserQuestion, tool/connection state-visibility reads, listAiHubTasks, listAiHubPersonalAgents, openAiHubPersonalAgentTab, Copilot specialist delegates
    - BUILD pinned tools: all subagent delegates (Copilot + AI-hub-owned + manager), openResourceTab (with artifact recorder), openWorkflowChatTab, listChatWorkflows, runChatWorkflow, createConnection, selectConnection, attachTaskTool, removeTaskTool, askUserQuestion, listAiHubTasks, createAssetFile, getAssetFileContent, listAssetFiles, plus personal-agent tab tools
    - Searchable catalog (ASK): read project/workflow/component/task/task-dispatcher tools + demoted listApiCollections
    - Searchable catalog (BUILD): project/workflow/component/task/task-dispatcher/script/cluster-element tools + demoted cloneAssetFile, createWorkflowChat
    ...
    - LazyToolCallingManager + AiHubTaskBindingToolCallbackResolver resolve attached task-tool bindings lazily
...
- **Specialist Subagents (AiHubAgentType)** [EE] — One-shot ChatClients registered as delegate tools, each with its own *Configuration + prompt resource. AiHubAgentType enumerates: AI_HUB_ASK, AI_HUB_BUILD, AI_HUB (fallback), FILES (fallback), RESEARCH, DATA_ANALYST, IMAGE_GENERATOR, SLIDE_BUILDER, WORKFLOW_BUILDER (superseded by workflow_editor_agent), MCP_MANAGER, PERSONAL_AGENT_MANAGER, DEPLOYMENT_MANAGER, API_COLLECTION_MANAGER.
    ...
    - Manager subagents via ManagerSubAgentToolCallback: mcp_manager (MCP project/server lifecycle incl. createMcpProject/listMcpProjectWorkflows/updateMcpProjectWorkflowParameters/createMcpServer/updateMcpServer, prompt_mcp_manager.txt), personal_agent_manager (create/update/delete/clone personal agents), deployment_manager (create/update/toggle/rollback/delete project deployments, promoteWorkflow), api_collection_manager (create/clone/list API collections)
    - Copilot specialist delegates (registerCopilotSubAgentToolCallbacks): skills_agent, context_store_agent, knowledge_base_agent, data_table_agent, cluster_element_agent, code_editor_agent, workflow_editor_agent, workflow_execution_agent, converter_agent (BUILD-only), custom_component_agent, code_workflow_agent — each gated on its ChatClient bean (Copilot enable flag independent of AI Hub)
    - Manager subagents also contributed to the management MCP server (AiHubManagerMcpContributorConfiguration) wrapped in WorkspaceScopedManagerToolCallback (workspaceId auto-select / workspace_required error)
    - Delegates forward parent ToolContext; wrapped in ProgressReportingToolCallback on the chat surface only
- **AG-UI Streaming Protocol and Runtime Providers** [EE] — Server streams turns over the AG-UI protocol (AgUiStreamBridge, AiHubChatStreamer) via SSE; the client selects a runtime per task kind — AiHubRuntimeProvider for STANDARD/PERSONAL_AGENT (AG-UI runAgent) and a webhook-stream handler for WORKFLOW_CHAT.
...
- **Artifact Generation** [EE] — GenerateArtifactToolCallback plus an ArtifactGeneratorRegistry of format-specific generators lets the agent produce downloadable/viewable artifacts stored as asset files.
    ...
    - Asset-file tools: createAssetFile, createBinaryAssetFile, getAssetFileContent, listAssetFiles, cloneAssetFile (catalog)
...
- **Task Artifacts and Audit Trail** [EE] — Every notable agent output or user-attached reference is recorded as an AiHubTaskArtifact row (kinds/status enums), surfaced in the sidebar and queryable workspace-wide by admins; a separate audit aspect publishes AiHubAuditEvents.
    - Artifact recorders: AiHubTaskArtifactRecorderImpl, WorkflowArtifactRecorderImpl, AiHubToolMutationArtifactRecorder
    - Per-task artifact listing (aiHubTaskArtifactsByAiHubTask)
    - Admin-only paginated workspace-wide artifact audit query (aiHubTaskArtifacts, with clamped page/size and pageClamped/sizeClamped surfacing)
    ...
    - Task asset-file relationships (AiHubTaskAssetFile / AiHubTaskAssetFileRelationship)
- **Progress Narration** [EE] — ProgressReportingToolCallback wraps subagent delegates on the chat surface and narrates their inner tool activity into the AG-UI stream via SubagentProgressChannel/SubagentProgressEmitter; the client shows live progress lines.
    ...
    - Not applied on the MCP surface (no AG-UI stream)
- **Workflow Chats** [EE] — Chat conversations bound to deployed chat-trigger workflows: WebhookBridgeAgent routes turns to webhook endpoints (/webhooks/<id> or /webhooks/<id>/sse) instead of the LLM, with a dedicated Workflow Chats page (tasks/WorkflowChats.tsx, WorkflowChatsList.tsx) and sidebar grouping by project deployment.
    - Chat-workflow discovery (listChatWorkflows tool, useWorkspaceChatWorkflowsQuery, home-panel picker)
    ...
    - Idempotent task creation per (workflow) pick; manual titles preserved (bridge bypasses LLM title generator)
    - Bulk archive of workflow-chat tasks
...
- **Security and Access Guards** [EE] — WorkspaceAccessGuard (ai-hub-api/security) enforces workspace-scoped ownership on tasks/agents; SecurityContextRehydrator re-establishes tenant + SecurityContext on tool-execution threads so @PreAuthorize facades work on Reactor schedulers; typed exceptions (NotFound/Forbidden/Conflict/TitleGenerationFailed, AuthorshipAlreadyAssigned).
...
mindmap
  root((AI Platform and Copilot))
    ...
    Copilot Specialist Agents and Subagents
      ...
      Delegate subagent tools workflow_editor_agent, code_edito…
    ...
    Management MCP Server and Tool Contributor SPI
      ...
      EE contributes mcp_manager, personal_agent_manager, deplo…
...
- **Copilot Specialist Agents and Subagents** — The copilot is a family of Spring AI agents (server/libs/ai/ai-copilot/ai-copilot-service/agent) with ASK/BUILD prompt variants per surface, plus specialist subagents exposed to parent agents as delegate tool callbacks (CopilotAgentType enum, ai-copilot-tool).
    ...
    - Delegate subagent tools: workflow_editor_agent, code_editor_agent, cluster_element_agent, converter_agent (BUILD-only), workflow_execution_agent, json_schema_builder_agent, sample_output_agent, context_store_agent, knowledge_base_agent, data_table_agent, skills_agent, custom_component_agent, code_workflow_agent
...
- **Management MCP Server & Tool Contributor SPI** — A management MCP server at /api/management/{secretKey}/mcp (server/libs/ai/ai-mcp) whose toolset is assembled from McpServerToolCallbackContributor SPI beans (ai-mcp-server-api), keeping the CE server free of EE imports while EE contributes manager subagents.
    ...
    - EE contributes mcp_manager, personal_agent_manager, deployment_manager, api_collection_manager subagents wrapped in WorkspaceScopedManagerToolCallback
    - Optional workspaceId tool input: sole workspace auto-selects, multiple returns workspace_required error listing candidates
    - API-key security configurer for the management endpoint
...
mindmap
  root((Embedded iPaaS))
    ...
    Embedded Public REST API
      ...
      Swagger/OpenAPI grouping via embedded-swagger SwaggerConf…
...
- **Embedded Public REST API** [EE] — OpenAPI-defined public surface (embedded-configuration-public-rest/openapi.yaml, embedded-execution-public-rest, embedded-webhook-public-rest) consumed by customer backends and the frontend SDK, with parallel /{externalUserId}/... variants for API-key callers acting on behalf of a user.
    ...
    - Swagger/OpenAPI grouping via embedded-swagger SwaggerConfiguration
...
mindmap
  root((Platform Services))
    ...
    Platform Shared Kernel and API Docs
      ...
      SpringDoc grouped API docs SwaggerConfiguration
    Workflow Editor Backend Definitions, Test, Environments
    ...
    Automation API Documentation
      Grouped OpenAPI/Swagger definitions for automation APIs
```
...
- **Knowledge Base Engine** — platform-knowledge-base implements RAG document management: knowledge bases, documents, chunks, and tags, with a message-driven ETL worker that reads documents (incl. OCR), splits them into overlapping token chunks, embeds them, and writes to the pgvector store; sources support scheduled sync with tombstone strategies.
    ...
    - Audit events (KnowledgeBaseAuditPublisher)
    - Source sync job listener updating source status
- **Platform Shared Kernel and API Docs** — platform-api holds cross-cutting primitives (edition conditionals, WorkflowExecutionId, base property/option models, security utils); platform-rest holds shared OpenAPI component schemas; platform-swagger configures the Swagger UI groupings for the REST APIs.
...
- **Automation Connection Glue** — automation-connection contains a ConnectionAfterSaveEventListener that reacts to connection saves on the automation side (e.g., wiring workspace association after creation).
    - Connection after-save event listener
- **Automation API Documentation** — automation-swagger registers the SpringDoc grouped-OpenAPI configuration for the automation REST API surface (projects, deployments, connections, workflows, executions).
    - Grouped OpenAPI/Swagger definitions for automation APIs
...
mindmap
  root((Enterprise and Microservices))
    ...
    Embedded iPaaS EE Modules owned by another explorer
      ...
      embedded-security + embedded-swagger
...
- **Embedded iPaaS EE Modules (owned by another explorer)** [EE] — server/ee/libs/embedded contains the embedded iPaaS product surface: integration configuration, connected users, unified API, embedded execution/webhook/workflow, embedded AI (copilot, MCP server), embedded security, swagger, plus the embedded-workflow-builder and app-event EE components. Noted for completeness; detailed inventory belongs to the embedded-area explorer.
    ...
    - embedded-security + embedded-swagger
    - embedded-workflow-builder and app-event components
- **AI Hub and AI Copilot Modules (owned by another explorer)** [EE] — server/ee/libs/ai holds ai-hub (workspace AI hub: tasks, personal agents, artifacts, MCP servers, schedules) and ai-copilot (workflow-description and property copilots); automation-ai-copilot/automation-ai-tool provide copilot tool wiring. Noted for completeness; detailed inventory belongs to the AI-area explorer.
    - ai-hub (api/graphql/rest/service)
    - ai-copilot (api/graphql/rest/service)
    - automation-ai-copilot and automation-ai-tool
...
mindmap
  root((Developer Surface and Deployment))
    ...
    Embedded REST API
      ...
      embedded-swagger OpenAPI groups frontend/internal/public
    Platform REST API
    ...
    GraphQL API
      ...
      ai/aihub artifact, mcp-server, personal-agent, task, task…
      ai/auto-memory
...
- **Automation REST API** — REST surface for the automation (iPaaS) product: internal API at /api/automation/internal/** and versioned public API at /api/automation/v1/** (automation-configuration-public-rest, EE), covering projects, deployments, workflows, executions, asset files.
    ...
    - OpenAPI groups automation-internal / automation-public via automation-swagger
- **Embedded REST API** [EE] — EE REST modules for the embedded iPaaS: internal (/api/embedded/internal/**), public (/api/embedded/v1/**), and frontend (/api/embedded/frontend/v1/**) API groups spanning configuration, connected users, executions, webhooks, unified API, and security.
    ...
    - embedded-workflow-execution-rest
    - embedded-swagger OpenAPI groups (frontend/internal/public)
- **Platform REST API** — CE REST modules for shared platform services: configuration, connections, workflow execution/test, users, tags, categories, notifications, knowledge base, AI skills, and webhooks; grouped as platform-internal in Swagger.
    - platform-configuration-rest
    - platform-connection-rest
    - platform-workflow-execution-rest
    - platform-workflow-test-rest
...
- **GraphQL API** — Internal GraphQL API consumed by the client, with operation domains under client/src/graphql covering AI, automation, embedded, platform, audit, and code-workflow features; codegen regenerates a typed middleware client.
    ...
    - ai/aihub (artifact, mcp-server, personal-agent, task, task-tool, workspace-settings)
    - ai/auto-memory
    - ai/skill (AI skills CRUD + generate-from-instructions)
...
- **OpenAPI / Swagger Documentation** — SpringDoc grouped OpenAPI definitions per API surface: automation-internal/public, platform-internal, and embedded frontend/internal/public groups, plus a docs-site openapi content section rendering the specs.
    - automation-swagger (CE)
    - platform-swagger (CE)
    - embedded-swagger (EE)
    - docs/content/docs/openapi reference pages
...
- **Documentation Site** — Next.js 16 + Fumadocs docs site (docs/) with MDX content, generated component reference, llms.txt/llms-full.txt AI-readable exports, OG image generation, and sitemap.
    - Getting Started
    - Automation (build, connect-data, deploy, monitor, AI Hub, MCP servers, personal agents, data tables, knowledge base, API platform, workflow chats, templates, asset files)
...
- **Design Spec Library** — docs/superpowers/specs holds ~180 dated design specs documenting shipped features — the authoritative evidence trail for AI Hub phases, AI Gateway gaps 1-17, voice, MCP auth, security hardening, ELK layout engine phases, code workflows, and connection visibility.
    ...
    - AI Hub: shell + phases 2-17, mothership parity, personal agents (model selection, scheduling, composer resources), voice (path B, TTS, browser tier1, cost/compliance, production readiness, widget), workflow-execution tabs, property-options lookup/select, skill panel + artifact parity, auto-memory resource fork, delegate-to-copilot subagents
