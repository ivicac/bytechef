...
## Scaffolding and deploying with the CLI
...
The CLI also deploys a built component - single-file JavaScript/Python/Ruby, or a Java `.jar` - to a running instance:

```bash
bytechef component deploy --file ./my-component.js
```

The profile token used for `component deploy` must be an admin platform API key, `.jar` deployment additionally requires `java-enabled` on the server, and the file extension selects the language server-side. Like an upload or an editor publish, this deploys immediately.
