...
| Scheduler | `scheduler-app` | Schedule and polling-trigger service; Quartz (JDBC job store) or db-scheduler, selected by `bytechef.scheduler.provider`. |
...
| `bytechef.scheduler.provider` | `quartz` (also `db-scheduler`, `aws`) |
...
**Switching from Quartz.** The first time `db-scheduler` becomes the active provider against a given
database, a `quartz-import` task copies live Quartz jobs (schedule and polling triggers, OAuth2 token
refreshes, dynamic-webhook refreshes, suspended-task resumes) into `scheduled_tasks`, preserving each
job's next fire time, and never modifies Quartz rows. It runs **once per database, not on every
startup**: `quartz-import` is itself a db-scheduler recurring task on a ~10-year schedule, so
db-scheduler only schedules it when no row for it exists yet, and the surviving row is the completion
marker. It logs one summary line, `Quartz import: scanned=…`. Disable the importer entirely with
`bytechef.scheduler.db-scheduler.importer.enabled: false`.
...
To force a re-import against the same database, delete the `quartz-import` row from
`scheduled_tasks` (for example through db-scheduler-ui, when it is not running in read-only mode) and
restart.

**Inspecting schedules.** The `db-scheduler-ui` dashboard ships in `server-app` (the monolith) only,
at `/db-scheduler` for users with the `SYSTEM_ADMIN` authority (read-only by default;
`db-scheduler-ui.read-only: false` enables re-run and delete). It is not wired into `scheduler-app` -
that app has no Spring Security stack to gate the dashboard behind, so its config keeps
`bytechef.scheduler.db-scheduler.ui.enabled: false` regardless of provider. In a distributed
deployment, inspect `db-scheduler` schedules from a monolith instance pointed at the same database,
or query `scheduled_tasks` directly.
