...
An agent belongs to the project you choose, alongside that project's workflows. The workflow an agent generates is its own, so it never also appears under **Workflows** - the two lists are disjoint.
