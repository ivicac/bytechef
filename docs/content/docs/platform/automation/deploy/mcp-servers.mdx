...
![Create MCP Server dialog](mcp-servers/mcp-create-server-dialog.png)
...
![Tool properties panel for a component action, with Tool Name and Tool Description above the action's own parameters](mcp-servers/mcp-component-tool-properties.png)
...
![Workflow tool mapping panel showing Tool Name, Tool Description and one field per trigger input](mcp-servers/mcp-workflow-tool-mapping.png)
...
<Callout type="warn" title="Why a workflow might not appear in the tool list">
    A workflow is silently skipped when the server lists its tools if either of two things is true:
    ...
    A missing tool name is **not** one of them - the name falls back to the workflow's own name.
</Callout>
