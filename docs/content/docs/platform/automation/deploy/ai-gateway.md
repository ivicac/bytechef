...
- **Detection no longer fires on every long number.** Each match is scored by how distinctive its
  shape is - an email address or a well-formed IBAN scores high; a bare run of digits that could just
  as easily be an order number, an invoice number, or a support-ticket ID scores low. Only matches at
  or above a confidence threshold get redacted, so a plain business identifier that happens to be the
  right length no longer gets masked and mislabelled as someone's SSN or bank number. The default
  threshold is tuned to keep catching real, structured PII while ignoring incidental digit runs, and
  it's tunable per workspace if your default needs adjusting - there's no settings-page control for it
  yet, so reach out if you need it changed.
