...
| `sharedConnectionIds` | Removed from `EmbeddedWorkflowBuilder` -- passing it is now a TypeScript error. (`AutomationHub` still accepts it for backward compatibility, but the server ignores it.) Mark a connection **Shared Connection** on the Connections page instead; every connected user in that environment can then reuse it. |
