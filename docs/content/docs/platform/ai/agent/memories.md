...
> **Enterprise Edition.** The Memories page is available only in the Enterprise Edition of ByteChef. It is reachable by any signed-in user (both the **Admin** and **User** roles); memories are scoped to your workspace and to the selected environment, so you only see the entries that belong to the workspace and environment you have open.

The **Memories** page is the management surface for **Auto Memory** — the durable, long-term facts an AI agent decides are worth keeping and reuses in every future conversation. Where the [Auto Memory tool](agent-utils#auto-memory) lets an agent *write* memories at runtime, this page lets you *read, correct, and prune* them.
...
|---|---|---|
| **Writing a memory** | An agent calls the [Auto Memory tool](agent-utils#auto-memory) mid-run | The agent, automatically |
| **Managing memories** | The Memories page (`/automation/ai/memories`) | You, by hand |

Auto Memory complements chat memory: chat memory replays the *current* conversation, while Auto Memory persists facts across *all* future conversations of the same scope. See [Chat memory vs Auto Memory](/platform/ai/agent#chat-memory-vs-auto-memory) for the distinction.
...
Memories are written by agents, not by hand. An agent stores a fact by calling the [Auto Memory tool](agent-utils#auto-memory) during a run. This page exists to review and maintain what agents have already learned.

#### Why don't I see any memories?
...
#### Why is the Name field greyed out when I edit?
...
#### How is this different from the AI Hub Memories page?

Both surfaces manage the same underlying Auto Memory capability. On the [AI Hub](/automation/ai-hub) the assistant saves facts per user; inside a deployed [AI Agent](/platform/ai/agent) the facts belong to the deployed workflow. This page manages the workspace-and-environment-scoped set for automation.
