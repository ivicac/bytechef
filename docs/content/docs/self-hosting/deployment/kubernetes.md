...
When running more than one replica, align the multi-instance settings described in [Manage Instance](/self-hosting/configuration/manage-instance#running-multiple-instances) — shared encryption key, a shared message broker and cache, and coordinated schema migrations.
...
These are the same probes documented under [Observability](/self-hosting/observability).
...
Schema migrations run automatically on startup. Back up the database first, and see [Manage Instance](/self-hosting/configuration/manage-instance) for upgrade and backup guidance.
