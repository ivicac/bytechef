...
- **Email notifications and alert rules are monolith-only for now**: job-status notifications work
  distributed for the webhook and Slack channels — `coordinator-app` resolves delivery targets
  through `configuration-app`'s `/remote/notification-service` endpoints and delivers with the
  shared CE transports — but the email channel warn-skips on `coordinator-app` (no mail sender is
  wired there), and workflow alert rules still require the monolith deployment.
