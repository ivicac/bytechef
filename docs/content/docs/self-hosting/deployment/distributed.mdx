...
(`bytechef.workflow.execution.retention.*`) can find expired runs remotely but currently skips
deletion in the distributed set, because the cascading job delete is not yet exposed through the
remote facade — retention purging is effectively monolith-only for now.
...
- **Retention purging is monolith-only for now**: the retention monitor's cascading job delete is
  not implemented in the remote job facade client, so expired execution history is only purged in
  the monolith deployment (see above).
