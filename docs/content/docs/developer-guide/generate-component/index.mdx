...
1. **[Initial setup](/developer-guide/generate-component/initial-setup)** — create the component package and register it in the Gradle build.
2. **[OpenAPI specification](/developer-guide/generate-component/open-api-specification)** — add your `openapi.yaml` and run the CLI `component init` command to generate the component.
3. **[Customize the component](/developer-guide/generate-component/customize-component)** — set the icon and category, adjust the connection and actions, and enable dynamic options, dynamic properties, dynamic output, and AI Agent tools via OpenAPI extensions.
4. **[Create a trigger](/developer-guide/generate-component/create-trigger)** — triggers are not generated from OpenAPI, so add them manually and wire them into the generated handler.
