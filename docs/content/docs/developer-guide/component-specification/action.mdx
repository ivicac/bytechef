...
- `perform(PerformFunction perform)` - Defines the action logic with access to the configured connection. Several overloads exist for specialized cases (`CallableResponsePerformFunction`, `WebhookResponsePerformFunction`, `StreamPerformFunction`), all sharing the same three-argument shape below. Actions no longer stream over WebSockets — only a Voice Agent cluster element can return a `WebSocketHandler`, and only the voice session engine invokes one.
