...
The Developer Guide is for engineers who want to extend ByteChef with new **components** (connectors) or understand how the platform runs the workflows they power. Everything here builds on the same Java Component SDK (`sdks/backend/java`) that ships the 180+ built-in components under `server/libs/modules/components/`.
