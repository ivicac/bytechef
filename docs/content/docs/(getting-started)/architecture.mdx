...
**API & Integration Layer** : This layer handles communication with external services and applications. It provides comprehensive REST and GraphQL APIs that allow seamless integration with over 200+ pre-built connectors and custom integrations through our component system.
...
**Component Interface** : Standardized interface that all components implement, ensuring consistency across 200+ integrations. Each component defines its actions, triggers, and required properties.
...
**Authentication** : Supports multiple authentication methods including OAuth 2.0, API keys, social login, and single sign-on (SSO) integration (Enterprise Edition).

**Authorization** : RBAC (Role-Based Access Control) with workspace-scoped roles controls what users can access and modify.
...
**Message Broker** : An in-memory broker by default, with support for Redis, RabbitMQ, Kafka, JMS, AMQP, and AWS SQS for asynchronous job processing and reliable message delivery.
...
For information on deploying ByteChef in your specific environment, see the [Self-Hosting](/self-hosting/configuration/environment-variables) section. For details on building custom integrations, visit the [Developer Guide](/developer-guide).