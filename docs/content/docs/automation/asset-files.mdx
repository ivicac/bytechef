...
- **Markdown** renders as formatted text; switch to the **Edit** tab to change the source.
- **CSV** renders as a table (first 500 rows).
- **HTML** renders in a sandboxed frame — scripts are disabled — with a fullscreen button.
- Other **text and code** files open in an editor with syntax highlighting.
- **Images** and **PDFs** render inline; everything else offers a download.
