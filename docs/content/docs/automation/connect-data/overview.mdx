...
<Callout type="info">
  If your administrator has configured an external secret backend, the create dialog shows a **Credential storage** selector. Choosing a non-default store lets you toggle **Register existing credential** and reference a secret that already lives in that store by its path or UUID, instead of typing the credentials into ByteChef. This selector only appears when more than one credential store is available, and it cannot be changed after the connection is created.
</Callout>
...
<Callout type="info">
  In the Enterprise Edition, connections also carry a **visibility** scope. A scope badge (Private, Workspace, or Organization) appears next to the connection, and admins can share a private connection with the whole workspace (**Share with workspace**) or make a shared connection private again (**Make private**) from the ellipsis menu.
</Callout>
