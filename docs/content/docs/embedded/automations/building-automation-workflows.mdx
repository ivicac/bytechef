...
<Callout title="Enterprise Edition only">
    Automation workflows and the embeddable builder are part of ByteChef **Embedded**, which requires an Enterprise Edition deployment (`bytechef.edition=ee`).
</Callout>

There are two workflow-building surfaces in Embedded, and they serve different people:

- The **Automation Workflow editor** is where **you** author the workflow templates that ship with your product. It is opened from the [Automations](/embedded/automations) page and edits templates in your workspace, against your workspace connections.
- The **Workflow Builder** is the **white-label, embeddable** editor you drop into your own app so a **connected user** can build and refine their own workflow against **their own** connections. It has no ByteChef chrome around it — see [White-Label Execution](/embedded/white-label-execution).

Both surfaces render the same drag-and-drop canvas as the standard automation editor, so the core mechanics — adding triggers and actions, wiring data pills, flow controls — are documented once under [Building workflows](/automation/build/workflows). This page focuses on what is specific to the two embedded surfaces.

---

## The Automation Workflow editor

Open the editor by clicking a workflow template (or a project name) on the [Automations](/embedded/automations) page. The URL is `/embedded/automation-workflows/{workflowUuid}/editor`.
...
For AI-driven building of a connected user's workflows from chat, see [Workflow Builder Tools](/embedded/workflow-builder-tools). For how execution stays isolated and white-labeled, see [White-Label Execution](/embedded/white-label-execution).

## See also

- [Automations](/embedded/automations) — the projects and templates list this editor opens from.
- [Building workflows](/automation/build/workflows) — the shared canvas mechanics used by both surfaces.
- [White-Label Execution](/embedded/white-label-execution) — how the builder is embedded and branded in your product.
- [Workflow Builder Tools](/embedded/workflow-builder-tools) — building a connected user's workflows from an AI agent.
