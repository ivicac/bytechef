...
3. Enter the event's **Schema** as JSON — the structure of the payload your application will send. The schema's properties become variables your workflows can read.
4. Click **Save**.
...
The connected user is identified by the JWT `sub` claim. ByteChef looks up that user's enabled integration instances and starts an execution for every workflow whose trigger is the **App Event** trigger. The endpoint takes no request body — per-event selection is configured on each workflow's trigger, not in the request.
