...
| User-creatable fields | `userCanCreateFields` lets users invent an application field not in your configured set. |
...
Design time (workflow builder)
  Workflow input  type = Field Mapping
    test value = static mapObjectFields-shaped JSON
      ...
      └─ applicationFields[]       → one data pill each
...
| `applicationFields.userCanCreateFields` | `boolean` | no | Allow inventing an application field not in `fields`. |
