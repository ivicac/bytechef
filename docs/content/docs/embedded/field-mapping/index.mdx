...
Runtime (end user, Connect dialog)
  useConnectDialog({ mapObjectFields: { <objectName>: { objectTypes, integrationFields, fields, ... } } })
...
| `fields` | `OptionType[]` | no | Your application's fields (the mappable left column). `applicationFields` is accepted as an alias. |
| `defaultFields` | `string[]` | no | `value`s of the rows visible initially. `[]` shows none; omitting shows all. |
| `userCanRemoveMappings` | `boolean` | no | Allow per-row remove and re-add. |
| `userCanCreateFields` | `boolean` | no | Allow inventing an application field not in `fields`. |
