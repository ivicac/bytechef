...
- [ ] blog.bytechef.io (76/100, https://is-agentic.com/scan/blog.bytechef.io): give the 404 page a short markdown body pointing at the sitemap and llms.txt (the status is already 404); pass "content without JavaScript" with one clear H1 and sequential heading levels on the homepage (raw HTML already carries 1,816 characters of text, so the structure is what fails); serve markdown on `Accept: text/markdown` with `Vary: Accept, Accept-Encoding` (today it returns HTML); add `llms.txt` with a "when to use ByteChef" section (today 404). The MCP check does not apply to a blog, ignore it. (Ivica, bytechef-blog repo)
