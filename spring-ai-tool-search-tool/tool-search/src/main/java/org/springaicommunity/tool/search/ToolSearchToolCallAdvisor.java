...
public class ToolSearchToolCallAdvisor extends ToolCallAdvisor {
	...
	protected ToolSearchToolCallAdvisor(ToolCallingManager toolCallingManager, int advisorOrder,
		...
		// ByteChef temp fork: M7 widened ToolCallAdvisor's constructor from
		// (ToolCallingManager, int) to (ToolCallingManager, int, boolean, boolean).
		// Pass M7 defaults to preserve upstream behavior: conversationHistoryEnabled=true,
		// streamToolCallResponses=false (both match the M7 Builder defaults).
		super(toolCallingManager, advisorOrder, true, false);
	...
	protected ChatClientRequest doBeforeCall(ChatClientRequest chatClientRequest, CallAdvisorChain callAdvisorChain) {
		...
		if (chatClientRequest.prompt().getOptions() instanceof ToolCallingChatOptions toolOptions) {
			...
			// ByteChef temp fork: M7 made ToolCallingChatOptions immutable. The
			// upstream pattern of copy() + setToolCallbacks/setToolNames/setToolContext
			// is gone; all mutation now flows through the Builder via .mutate().
