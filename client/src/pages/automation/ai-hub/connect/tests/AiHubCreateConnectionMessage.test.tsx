...
const renderMessage = async (data = DEFAULT_DATA) => {
    const {default: AiHubCreateConnectionMessage} = await import('../AiHubCreateConnectionMessage');

    return render(
        <AiHubCreateConnectionMessage data={data} name="create-connection" status={{type: 'complete'}} type="data" />
    );
};

describe('AiHubCreateConnectionMessage', () => {
    beforeEach(() => {
