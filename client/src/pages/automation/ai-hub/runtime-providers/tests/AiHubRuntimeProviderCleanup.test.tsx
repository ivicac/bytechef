...
describe('cleanupForTaskChange', () => {
    beforeEach(() => {
        aiHubToolCallStore.setState({toolCalls: {}});
        vi.restoreAllMocks();
        vi.clearAllMocks();
    });

    it('calls resetForTask with the previousTaskId snapshot', () => {
        const resetSpy = vi.spyOn(aiHubToolCallStore.getState(), 'resetForTask');
    ...
    it('does NOT mark running tool calls as aborted on task switch', () => {
        const failAllRunningSpy = vi.spyOn(aiHubToolCallStore.getState(), 'failAllRunning');
