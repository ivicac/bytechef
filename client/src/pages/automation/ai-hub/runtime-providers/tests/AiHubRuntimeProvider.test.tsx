...
import {MODE} from '@/pages/automation/ai-hub/stores/useAiHubStore';
...
vi.mock('@/pages/automation/ai-hub/tasks/api/tasks.api', () => ({
...
const {generateAiHubTaskTitle, patchTask} = await import('@/pages/automation/ai-hub/tasks/api/tasks.api');
...
describe('buildAiHubSubscriber', () => {
    ...
    it('switches task when openAiHubPersonalAgentTab tool result arrives', async () => {
        ...
        const {aiHubTasksStore} = await import('@/pages/automation/ai-hub/tasks/stores/useAiHubTasksStore');
        const {aiHubStore} = await import('@/pages/automation/ai-hub/stores/useAiHubStore');
    ...
    it('runChatWorkflow result without streamUrl or responseUrl flips card to error and sets retryable error', () => {
        ...
        const entry = aiHubToolCallStore.getState().toolCalls['call-rcw-empty'];
...
describe('tool-call store routing in buildAiHubSubscriber', () => {
    ...
    it('onToolCallStartEvent registers a running entry tagged with the assistant message index', () => {
        ...
        const entry = aiHubToolCallStore.getState().toolCalls['call-x'];
    ...
    it('onToolCallEndEvent merges the args into the entry', () => {
        ...
        expect(aiHubToolCallStore.getState().toolCalls['call-args'].args).toEqual({name: 'standup'});
    });
    ...
    it('onToolCallResultEvent flips the entry to success and stores the parsed result', () => {
        ...
        const entry = aiHubToolCallStore.getState().toolCalls['call-r'];
    ...
    it('onToolCallResultEvent flips to error when the result has an error field', () => {
        ...
        expect(aiHubToolCallStore.getState().toolCalls['call-err'].status).toBe('error');
    });
    ...
    it('runChatWorkflow streaming routes the SSE chunks into the matching tool-call progressive output', () => {
        ...
        expect(aiHubToolCallStore.getState().toolCalls['call-rcw'].progressiveOutput).toBe('Step 1 done');
    });
    ...
    it('subagent-progress CustomEvent routes into the matching subagent tool-call progress list', () => {
        ...
        const entry = aiHubToolCallStore.getState().toolCalls['call-sub'];
