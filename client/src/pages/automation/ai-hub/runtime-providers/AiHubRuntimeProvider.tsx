import {aiHubComposerStore} from '@/pages/automation/ai-hub/composer/stores/useAiHubComposerStore';
import {
    ToolCallEntryI,
    aiHubToolCallStore,
    isRunningToolCall,
    useAiHubToolCallStore,
} from '@/pages/automation/ai-hub/messages/stores/useAiHubToolCallStore';
import {aiHubProgressStore} from '@/pages/automation/ai-hub/progress/stores/useAiHubProgressStore';
import {aiHubRetryableErrorStore} from '@/pages/automation/ai-hub/retry/stores/useAiHubRetryableErrorStore';
import {attachToInFlightRun, probeInFlightStatus} from '@/pages/automation/ai-hub/runtime-providers/inFlightRunClient';
import {
    aiHubRunStateStore,
    isTaskRunning,
...
} from '@/pages/automation/ai-hub/runtime-providers/workflowStreamHandler';
...
import {createAiHubTask, generateAiHubTaskTitle, patchTask} from '@/pages/automation/ai-hub/tasks/api/tasks.api';
...
type OpenTaskTabResultType =
    | {taskId: number; opened: true; threadId: string; title: string | null}
    | {error: string; opened: false};
...
const parseJson = <T,>(content: string, contextLabel: string): T | null => {
    try {
        return JSON.parse(content) as T;
    } catch (parseError) {
        // Surface malformed tool-result payloads instead of swallowing them — every caller treats `null` as
        // "best-effort skip" so without a log a regression in the LLM/tool serializer disappears entirely.
 ...
 * nothing matches.
 */
const humanizeAgentErrorMessage = (raw: string): string => {
    ...
    return {
        onCustomEvent: ({event}) => {
            ...
            if (payload && typeof payload.subagentName === 'string' && typeof payload.text === 'string') {
                ...
                const matching = aiHubToolCallStore
                    .getState()
                    .findRunningToolCallByName(payload.subagentName, subscriberTaskId);

                if (matching) {
                    aiHubToolCallStore.getState().addProgress(matching.toolCallId, payload.text);
                }
            }
        },
        onRunErrorEvent: ({event}) => {
            ...
            // Surface the server-side error inline as a distinct red callout bubble (see AiHubRunErrorMessage).
        ...
        onToolCallEndEvent: ({event, toolCallArgs}) => {
            ...
            aiHubToolCallStore.getState().updateToolCallArgs(event.toolCallId, toolCallArgs ?? {});
        },
        onToolCallResultEvent: ({event}) => {
            ...
            aiHubToolCallStore.getState().completeToolCall(event.toolCallId, parsedAny ?? event.content, isError);
            ...
            if (toolCallName === 'openFileTab') {
                ...
                aiHubToolCallStore.setState((state) => {
                ...
                const sinkAppender = (text: string) => {
                    aiHubToolCallStore.getState().appendProgressiveOutput(targetToolCallId, text);
                };
                ...
                const onWorkflowStreamComplete = (info: WorkflowStreamCompleteType) => {
                    try {
                        const finalEntry = aiHubToolCallStore.getState().toolCalls[targetToolCallId];
                        ...
                        aiHubToolCallStore.getState().completeToolCall(targetToolCallId, finalResult, false);
                    } finally {
                        workflowStreamLifecycle?.onSettle();
                    }
                };
                ...
                if (parsed.streaming && parsed.streamUrl) {
                    ...
                    aiHubToolCallStore.getState().completeToolCall(targetToolCallId, {error: errorMessage}, true);
            ...
            } else if (toolCallName === 'createConnection') {
                const parsed = parseJson<CreateConnectionResultI>(event.content, 'createConnection result');

                if (!parsed || parsed.kind !== 'create-connection' || !parsed.componentName) {
                    // Without a surface here the malformed payload would silently disappear: no toast, no
                    // banner, no card update. Flip the tool-call card to error and raise a retryable error so
                    // the user can re-issue the prompt instead of staring at a stalled request.
            ...
            } else if (toolCallName === 'selectConnection') {
                const parsed = parseJson<SelectConnectionResultI>(event.content, 'selectConnection result');

                if (!parsed || parsed.kind !== 'select-connection' || !parsed.componentName) {
                    // Same shape as the createConnection malformed-payload branch: flip the card + raise the
                    // retryable error so the user has a recovery path instead of an inert dropdown.
                    const errorMessage = !parsed
            ...
            } else if (toolCallName === 'askUserQuestion') {
                const parsed = parseJson<AskUserQuestionResultI>(event.content, 'askUserQuestion result');
...
export const cleanupForTaskChange = (previousTaskId: string | undefined): void => {
    ...
    aiHubToolCallStore.getState().resetForTask(previousTaskId);
};
...
export const terminateOrphanRunningToolCalls = (
    ...
    aiHubToolCallStore.getState().failRunningInTask(taskId, {reason: 'Run finished with no result event'}, status);
};
...
export function AiHubRuntimeProvider({children}: Readonly<{children: ReactNode}>) {
    ...
    const toolCalls = useAiHubToolCallStore((state) => state.toolCalls);
    ...
    const onNew = async (message: AppendMessage) => {
        ...
        // Auto-create a DB task on the first message so the home view can flip to the thread view
        // (AiHub.tsx branches on currentTaskId). Skipping this would leave the user typing into
        // the home composer with no persisted task row, and the sidebar would never reflect the new chat.
        // The non-null assertion on `taskId` is safe here: the `if (!agent)` guard above already returned
        // early, and `agent` is built from `taskId` via useMemo — agent existing implies taskId set.
        ...
        // Generate the runId and publish it BEFORE setTaskRunning makes the Stop button appear, so a
        // Stop click always has a runId to send. handleCancelTurn forwards it to cancelAiHubRun so the
        // server can tombstone this exact run even if the Stop reaches the server first.
        ...
        // Sidebar pulse on the task's row so the user sees something is in flight even if the
        // chat thread isn't open. Clearing happens in onRunFinishedEvent / onRunErrorEvent; if the workflow
        // pauses with ask_user_question, the activity flips to 'paused' which the RUN_FINISHED handler
        // preserves so the indicator becomes "needs your answer".
        // Activity state is keyed by AG-UI thread id (the per-turn subscriber's identifier) — see the store
        // doc for why we picked thread id over the numeric row id. Use the taskId from useAiHubStore
        // here since that's the same thread id the subscriber will see.
        if (taskId != null) {
            aiHubTasksStore.getState().clearActivityState(taskId);
            aiHubTasksStore.getState().setActivityState(taskId, 'running');
        }
        ...
        try {
            ...
            if (isAbort) {
                ...
                aiHubToolCallStore.getState().failAllRunning({error: errorMessage});
                aiHubRetryableErrorStore.getState().setError({
                    errorMessage,
                    lastUserMessage: input,
                    toolName: 'aiHub',
    ...
    const onCancel = async () => {
        ...
        aiHubToolCallStore.getState().failAllRunning({reason: 'Cancelled by user'}, 'aborted');
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            aiHubToolCallStore.getState().failAllRunning({reason: 'Page closed'}, 'aborted');
        };
    }, []);
