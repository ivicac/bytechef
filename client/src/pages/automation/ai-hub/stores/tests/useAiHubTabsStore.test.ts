...
describe('useAiHubTabsStore', () => {
    ...
    describe('setActiveTaskId — home → task hand-off', () => {
        ...
        it('still snapshots and resets on task → task switches', () => {
            const {result} = renderHook(() => useAiHubTabsStore());

            // Land in conv-1 with one tab.
            act(() => {
                result.current.setActiveTaskId(1);
            ...
            // Switch to conv-2 with no prior snapshot — fresh empty view.
            act(() => {
            ...
            // Switch back to conv-1 — tab restored from snapshot.
            act(() => {
                result.current.setActiveTaskId(1);
            });
