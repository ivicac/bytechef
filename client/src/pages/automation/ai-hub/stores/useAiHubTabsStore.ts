 ...
 * restores the exact tabs the user had open + the right-panel-open state, instead of inheriting the tabs
 * from whatever task was most recently active. Pure in-session memory — not persisted to the
 * backend, so a hard reload still returns to a clean state.
 */
...
interface AiHubTabsStateI {
     ...
     * {@link rightPanelOpen}. `undefined` means home view. Updated via {@link setActiveTaskId},
     * which is responsible for snapshotting the previous task's tabs and restoring the new
     * task's tabs. */
    ...
    devtools(
        persist(
            (set) => ({
                 ...
                 * the previously open tabs + right-panel-open state for that task.
                 */
                    ...
                    set((state) => {
                        ...
                        if (restored == null && isHomeToTask) {
                            return {
                                ...
                                // openTabs / activeTabId / rightPanelOpen retained from current state.
                ...
                setTasksSidebarCollapsed: (collapsed) => set((state) => ({...state, tasksSidebarCollapsed: collapsed})),
