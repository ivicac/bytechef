...
import {
    type ResourcePickerSelectionI,
    type ResourcePickerToolsBranchI,
 ...
 *   - the composer's `onSelect` callback routing (non-workflow → handleSelect, workflow → handleSelectWorkflow), and
 *   - that the composer supplies a `toolsBranch`.
 *
 * ResourcePickerMenu itself is mocked so the captured `onSelect` / `toolsBranch` props can be driven
 * directly without rendering the real nested popover.
 */
...
const {resourcePickerOnSelectRef, resourcePickerToolsBranchRef} = vi.hoisted(() => ({
    resourcePickerOnSelectRef: {current: null as ((selection: ResourcePickerSelectionI) => void) | null},
    resourcePickerToolsBranchRef: {current: null as ResourcePickerToolsBranchI | undefined | null},
}));
...
vi.mock('@/pages/automation/ai-hub/resource-picker/ResourcePickerMenu', () => ({
    default: ({
        onSelect,
        toolsBranch,
        trigger,
        ...
        resourcePickerToolsBranchRef.current = toolsBranch;
...
vi.mock('@/pages/automation/ai-hub/tools/dialogs/TaskToolDialog', () => ({
    default: () => null,
}));
...
beforeEach(() => {
    ...
    resourcePickerToolsBranchRef.current = null;
});
...
describe('AiHubComposer ResourcePickerMenu wiring', () => {
    ...
    it('supplies a toolsBranch with renderRootItem and renderBranch functions', async () => {
        await renderComposer();

        const toolsBranch = resourcePickerToolsBranchRef.current;

        expect(toolsBranch).toBeDefined();
        expect(typeof toolsBranch?.renderRootItem).toBe('function');
        expect(typeof toolsBranch?.renderBranch).toBe('function');
    });
});
