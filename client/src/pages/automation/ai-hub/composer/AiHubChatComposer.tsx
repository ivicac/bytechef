...
import AiHubComposer from '@/pages/automation/ai-hub/composer/AiHubComposer';
import AiHubComposerDropZone from '@/pages/automation/ai-hub/composer/AiHubComposerDropZone';
import {
    ALLOWED_MIME_TYPES,
    useAiHubAttachmentUpload,
} from '@/pages/automation/ai-hub/composer/hooks/useAiHubAttachmentUpload';
import {
...
import {useAiHubTasksStore} from '@/pages/automation/ai-hub/tasks/stores/useAiHubTasksStore';
...
import {ArrowUpIcon, Loader2Icon, PaperclipIcon, RotateCcwIcon, SquareIcon, XIcon} from 'lucide-react';
...
const KIND_BADGE_CLASSES: Record<ReferencedResourceKindType, string> = {
    apiCollection: 'bg-pink-100 text-pink-700',
    dataTable: 'bg-green-100 text-green-700',
    file: 'bg-blue-100 text-blue-700',
    knowledgeBase: 'bg-purple-100 text-purple-700',
    mcpServer: 'bg-cyan-100 text-cyan-700',
    task: 'bg-slate-100 text-slate-700',
    workflow: 'bg-amber-100 text-amber-700',
    workflowExecution: 'bg-orange-100 text-orange-700',
};
...
const AiHubChatComposer = ({modelPicker}: AiHubChatComposerPropsI) => {
    ...
    const pushToTalk = usePushToTalk({
        ...
        transcribeUrl: '/api/automation/internal/transcribe',
    });
    ...
    // listener so any paste with files while this composer is mounted triggers an upload.
    useEffect(() => {
    ...
    return (
        ...
        //
        // `px-2` matches `AiHubThread`'s viewport padding (`overflow-x-hidden overflow-y-auto
        // px-2`) so the composer's left/right edges line up with the messages above. Earlier `px-4`
        // looked indented from the chat content because the thread viewport uses a smaller inset.
        // `px-6` when the resource panel is open keeps the same alignment with the thread viewport,
        // which mirrors the same conditional widening.
        <div
            className={twMerge(
                'mx-auto flex w-full max-w-[var(--thread-max-width,44rem)] flex-col gap-2 px-3 pb-2',
                rightPanelOpen && 'px-3'
            )}
        >
            <AiHubComposerDropZone onFilesDropped={handleFilesDropped}>
                <ComposerPrimitive.Root className="flex w-full flex-col rounded-2xl border border-border bg-muted/40 shadow-sm focus-within:border-foreground/20">
                        ...
                        <div className="flex flex-wrap gap-1.5 px-3 pt-2" data-testid="reference-chips">
                            ...
                            {uploads.map((uploadState) => {
                                ...
                                if (uploadState.status === 'error') {
                                    return (
                                        <Badge
                                            className="flex items-center gap-1 bg-red-100 px-2 py-0.5 text-xs font-normal text-red-700"
                                ...
                                return (
                                    <Badge
                                        className="flex items-center gap-1 bg-blue-50 px-2 py-0.5 text-xs font-normal text-blue-700"
                    ...
                    <div className="flex items-center justify-between gap-1 px-2 pb-2">
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                            ...
                            <MicButton
                                onClick={() =>
                                    pushToTalk.status === 'recording' ? pushToTalk.stop() : pushToTalk.start()
                                }
