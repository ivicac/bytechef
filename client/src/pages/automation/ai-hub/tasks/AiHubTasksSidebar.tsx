...
import {probeInFlightStatus} from '@/pages/automation/ai-hub/runtime-providers/inFlightRunClient';
import {aiHubRunStateStore} from '@/pages/automation/ai-hub/runtime-providers/stores/useAiHubRunStateStore';
import {aiHubTabsStore} from '@/pages/automation/ai-hub/stores/useAiHubTabsStore';
import {useLiveWorkflowLabel} from '@/pages/automation/ai-hub/tasks/hooks/useLiveWorkflowLabel';
...
const TasksSidebarSkeleton = () => (
    <>
        {Array.from({length: 2}).map((_, groupIndex) => (
            <div className="mb-4 px-2" key={groupIndex}>
...
const ArtifactRow = ({artifact, task, workspaceId}: ArtifactRowProps) => {
    ...
    const deleteMutation = useDeleteAiHubTaskArtifactMutation({
        ...
        // The artifact list query is keyed by ['TaskArtifactsByTask', ...] — invalidate by operation
        // prefix so the row disappears from the sidebar without needing the exact key shape react-query
        // built it under. Mirrors useRecordReferencedArtifacts' invalidation strategy.
        onSuccess: () => {
            queryClient.invalidateQueries({
                predicate: (query) => {
                    ...
                    return key[0] === 'taskArtifactsByTask';
                },
            });
        },
    });
    ...
    const relativeTime = useMemo(() => {
        try {
            ...
            console.warn('[AiHubTasksSidebar] Failed to format artifact.createdAt as relative time', {
...
const TaskItem = ({
     ...
     */
    const {data: artifacts} = useAiHubTaskArtifactsQuery(isExpanded ? task.id : undefined, workspaceId, isExpanded);
    ...
    return (
        <div>
                ...
                <div className="ml-1 flex shrink-0 items-center gap-0.5">
                    {titleGenerationError && (
                        <button
                            ...
                            className="rounded p-0.5 text-amber-600 hover:bg-muted"
            ...
            {isExpanded && <ArtifactList task={task} workspaceId={workspaceId} />}
        </div>
    );
};
...
const AiHubTasksSidebar = () => {
        ...
        const runProbe = () => {
            void probeInFlightStatus(visibleThreadIds).then((statusByThreadId) => {
                ...
                visibleThreadIds.forEach((threadId) => {
                    const isInFlight = statusByThreadId[threadId] ?? false;
                    const currentActivity = tasksState.taskActivity[threadId];
                    ...
                    // Server reports the run is no longer in flight. Clear a stale 'running' dot, but
                    // preserve 'paused' (workflow-chat ask_user_question) and defer to the local runtime
                    // when it claims the task — see the multi-line guard rationale above.
                    if (currentActivity === 'running' && !runState.runningByTask[threadId]) {
    ...
    const isOnMemories = pathname.startsWith('/automation/ai-hub/memories');
    const isOnArtifactHistory = pathname.startsWith('/automation/ai-hub/artifact-history');
    ...
    return (
        <div className="flex flex-col gap-2 p-2">
            ...
            <div className="flex flex-col gap-0.5">
                ...
                {/*
                 * Memories and Artifact History are full-page routes, same pattern as Personal Agents and
                 * Workflow Chats. They're grouped under a single "More" entry to keep the top-level menu compact —
                 * both are secondary destinations users reach occasionally, so collapsing them behind one row
                 * trims the always-visible list to the three primary entries above. The "More" row lights up
                 * active whenever either nested route is open, so the user still gets a "you're in here" cue.
                 * BrainIcon for Memories matches the icon used on the per-row memory affordances elsewhere in
                 * the surface; ScrollTextIcon for Artifact History reads as "log of recorded events".
                 */}

                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button
                            className={twMerge(menuItemClass(isOnMemories || isOnArtifactHistory), 'w-full text-left')}
                            type="button"
                        >
                    ...
                    <DropdownMenuContent align="start" className="w-48">
                        <DropdownMenuItem asChild>
                            <Link to="/automation/ai-hub/memories">
                                <BrainIcon />
                        ...
                        <DropdownMenuItem asChild>
                            <Link to="/automation/ai-hub/artifact-history">
                                <ScrollTextIcon />
