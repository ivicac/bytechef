...
// stays scoped to the four kinds the agent can mutate (files, workflows, data tables, knowledge bases).
 ...
 */
export default function useRecordReferencedArtifacts(taskId: number | undefined, workspaceId: number) {
    ...
    const recordMutation = useRecordReferencedAiHubTaskArtifactMutation({
        onSuccess: () => {
            // The artifact list query is keyed by ['TaskArtifactsByTask', ...] — match by
            // operation prefix so the invalidation works regardless of the exact arg shape react-query keys
            // by. The sidebar's artifact-list panel rerenders with the new row.
    ...
    }, [openTabs, taskId, tabsActiveTaskId, workspaceId]);
}
