...
import {ToolCallEntryI, useAiHubToolCallStore} from '@/pages/automation/ai-hub/messages/stores/useAiHubToolCallStore';
import {
    AlertCircleIcon,
    CheckCircle2Icon,
    ChevronDownIcon,
...
const StatusIcon = ({className, status}: StatusIconProps) => {
    ...
    if (status === 'error') {
        return <AlertCircleIcon className={twMerge('text-content-error-primary size-4', className)} />;
    }

    return <CheckCircle2Icon className={twMerge('size-4 text-emerald-600', className)} />;
};
...
const CollapsibleCard = ({children, expanded, header, onToggle}: CollapsibleCardProps) => {
    return (
        <div className="my-2 flex w-full flex-col rounded-lg border border-border bg-surface-neutral-secondary text-sm">
            <button
...
const RemoveTaskToolBody = ({entry}: SpecialRendererProps) => {
    ...
    return (
        <div className="flex flex-col gap-1 text-xs">
            <div className="flex items-center gap-2">
                <CheckCircle2Icon
                    className={twMerge('size-4', removed ? 'text-emerald-600' : 'text-muted-foreground')}
                />
...
const MemoryBody = ({entry}: SpecialRendererProps) => {
    ...
    return (
        <div className="flex flex-col gap-1 text-xs">
            ...
            {memoryType && (
                <div>
                    <Badge className="bg-purple-100 text-[10px] text-purple-700" styleType="outline-outline">
                        {memoryType}
                    </Badge>
                </div>
            )}
...
const AiHubToolCallRenderer = ({args, argsText, isError, result, toolCallId, toolName}: AiHubToolCallRendererProps) => {
    ...
    const liveEntry = useAiHubToolCallStore((state) => (toolCallId ? state.toolCalls[toolCallId] : undefined));
