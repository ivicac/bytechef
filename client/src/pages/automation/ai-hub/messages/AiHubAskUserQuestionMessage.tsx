...
import {useAiHubAskedQuestionsStore} from './stores/useAiHubAskedQuestionsStore';
 ...
 * single combined system message at the end. The wizard pattern fixes two prior UX bugs in the
 ...
 *
 * <p>
 * Answers are persisted in {@link useAiHubAskedQuestionsStore} keyed by a fingerprint over the whole
 ...
 * </p>
 */
const AiHubAskUserQuestionMessage = ({data}: DataMessagePartProps<AskUserQuestionDataI>) => {
    ...
    const isAnswered = useAiHubAskedQuestionsStore((state) => state.hasAnswered(fingerprint));
    const persistedAnswer = useAiHubAskedQuestionsStore((state) => state.getAnswer(fingerprint));
    const markAnswered = useAiHubAskedQuestionsStore((state) => state.markAnswered);
...
export default AiHubAskUserQuestionMessage;
