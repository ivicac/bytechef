...
describe('AiHubToolCallRenderer', () => {
    ...
    it('renders runChatWorkflow with per-step sections from the store progressive output', () => {
        aiHubToolCallStore.getState().startToolCall('call-rcw', 'runChatWorkflow', 0);
        aiHubToolCallStore.getState().appendProgressiveOutput('call-rcw', 'Step 1 ran fine\n\nStep 2 also ran');
    ...
    it('renders a subagent tool call as a progress breadcrumb', () => {
        aiHubToolCallStore.getState().startToolCall('call-r', 'research', 0);
        aiHubToolCallStore.getState().addProgress('call-r', 'Searching the web');
        aiHubToolCallStore.getState().addProgress('call-r', 'Synthesizing answer');
    ...
    it('subagent renders a starting placeholder when no progress events have arrived yet', () => {
        aiHubToolCallStore.getState().startToolCall('call-r', 'workflowBuilder', 0);
