...
/**
 * Surface query errors that would otherwise be silently swallowed. The AI Hub api/* helpers use raw fetch
 * and don't go through the global GraphQL interceptor, so a failed list/get request would show "0 items" with no
...
export function useReportQueryError(action: string, error: Error | null) {
    ...
    useEffect(() => {
        ...
        console.error(`[AiHub] ${action} failed:`, error);
