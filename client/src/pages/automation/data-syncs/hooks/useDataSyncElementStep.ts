 ...
 * one appears — a row can disappear under a live instance when the sync is reloaded or its element replaced
 * from elsewhere. The effect is keyed only on the element's identity (`id`/`componentName`/`operationName`),
 * which stays `undefined` for the whole time a user is picking a component/operation that has no element yet,
 * so this reset never fires mid-pick and never clobbers an in-progress selection. It is NOT what keeps the two
 * `kind`s apart: `DataSyncWizard` gives each step its own `key`, so this hook is mounted fresh per kind and its
 * `useState` initialisers read the right element from the first render.
 */
export default function useDataSyncElementStep({dataSync, kind}: UseDataSyncElementStepProps) {
    ...
    const connectionRequired = selectedComponentDefinition?.connectionRequired;
