...
import {
    ...
    useAiExperimentRunByTraceIdQuery,
...
const SPAN_TYPE_CLASSES: Record<string, string> = {
    [AiObservabilitySpanTypeEnum.Event]: 'bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300',
    [AiObservabilitySpanTypeEnum.Generation]: 'bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300',
    [AiObservabilitySpanTypeEnum.Span]: 'bg-gray-100 text-gray-800 dark:bg-gray-950 dark:text-gray-300',
    [AiObservabilitySpanTypeEnum.ToolCall]: 'bg-orange-100 text-orange-800 dark:bg-orange-950 dark:text-orange-300',
};
                    ...
                    <div className="mt-3 border-t pt-3 text-sm">
                        ...
                        {span.input && (
                            <div className="mt-3">
                                ...
                                <pre className="mt-1 max-h-40 overflow-y-auto rounded bg-muted p-2 text-xs whitespace-pre-wrap">
                                    {span.input}
                                </pre>
                            </div>
                        )}
                        ...
                        <div className="mt-3 border-t pt-3" onClick={(event) => event.stopPropagation()}>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    className="flex items-center gap-1 rounded-md border px-2 py-1 text-xs hover:bg-surface-success-secondary-hover"
...
const AiObservabilityTraceDetail = ({onBack, traceId}: AiObservabilityTraceDetailProps) => {
    ...
    // Reverse lookup: surface a "View source experiment" link when this trace was generated by an experiment
    // replay rather than a production API call. Returns null for production / OTLP / playground traces, so the
    // banner stays out of the way for the common case. The boundary check is server-side — see
    // AiExperimentGraphQlController.aiExperimentRunByTraceId.
    const {data: experimentRunData} = useAiExperimentRunByTraceIdQuery({traceId});

    const experimentRun = experimentRunData?.aiExperimentRunByTraceId;
    ...
    return (
        <div className="w-full px-2 2xl:mx-auto 2xl:w-4/5">
            ...
            {experimentRun != null && (
                <div className="mb-4 flex items-center gap-2 rounded-md border border-amber-300 bg-surface-warning-secondary px-4 py-3 text-sm dark:border-amber-700">
                    <FlaskConicalIcon className="size-4 shrink-0 text-content-warning-primary" />

                    <div className="flex-1">
                        <span className="font-medium">{`This trace was generated by an experiment replay.`}</span>

                        <span className="ml-2 text-muted-foreground">
                            {`Experiment #${experimentRun.experimentId} · run #${experimentRun.id} · dataset item #${experimentRun.datasetItemId}`}
                        </span>
                    </div>
                </div>
            )}
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        className="flex items-center gap-1 rounded-md border px-2 py-1 text-sm hover:bg-surface-success-secondary-hover disabled:cursor-not-allowed disabled:opacity-50"
