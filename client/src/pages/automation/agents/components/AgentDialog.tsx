...
const AgentDialog = ({
    ...
    // Only fetched when the dialog can actually offer the picker — editing an existing agent or opening it with a
    // locked project never renders the Select, so the query would otherwise fire without being used.
    const {data: projects} = useGetWorkspaceProjectsQuery({id: currentWorkspaceId}, !agent && !projectId);
        ...
        createAgentMutation.mutate(
            ...
            },
            {
                onSuccess: (data) => {
                    ...
                    // A "new project" create just created one behind the scenes — without this the projects list
                    // would keep showing its stale state until something else happened to refetch it.
                    queryClient.invalidateQueries({queryKey: ProjectKeys.filteredProjects({id: currentWorkspaceId})});

                    setOpen(false);

                    navigate(createdRedirectPath ?? getAgentPath(data.createAiAgent));
                },
            }
        );
                                            ...
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="new">
                                                        New project named after this agent
                                                    </SelectItem>
