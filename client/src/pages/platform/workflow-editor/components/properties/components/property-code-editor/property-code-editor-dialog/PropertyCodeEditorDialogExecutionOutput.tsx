...
const PropertyCodeEditorDialogExecutionOutput = () => {
    ...
    const renderContent = () => {
        ...
        if (scriptTestExecution.output) {
            const output = scriptTestExecution.output as Record<string, unknown>;

            const keys = Object.keys(output);

            const unwrappedOutput = keys.length === 1 && keys[0] === 'value' ? output.value : output;
            ...
            return <pre className="mt-2 text-xs">{String(unwrappedOutput)}</pre>;
        }
