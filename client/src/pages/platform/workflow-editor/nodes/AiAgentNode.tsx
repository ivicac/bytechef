...
const AiAgentNode = ({data, id}: {data: NodeDataType; id: string}) => {
    ...
    // Every cluster edge anchors to a `<elementType>-handle` source handle on the root, and React
    // Flow silently drops an edge whose named handle does not exist. On the main canvas a cluster
    // root renders as this component rather than as a `WorkflowNode`, so without these the whole box
    ...
    const canPaste = useMemo(
        () => !!copiedNode && !copiedNode.trigger && copiedWorkflowId === workflow.id,
