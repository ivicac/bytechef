...
export function toToolResultDataPart(toolCallName: string, eventContent: string): ToolResultDataPartType | undefined {
    ...
    // the tool name is `buildWorkflow` (or any other specialist) and none of the branches above match.
    // Keyed on the payload's own `kind` so one client change serves every specialist, present and future.
    //
