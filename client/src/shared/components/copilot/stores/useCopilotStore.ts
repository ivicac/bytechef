...
interface CopilotStateI {
    ...
    // Per-conversation LLM picker selection. Both null = use the workspace @Primary ChatModel. When set,
    // the client injects these into the AG-UI state on every chat request and the server's
    // CopilotChatClientResolver builds an override ChatClient. Selection is sticky across messages within
    // the same conversation; generateConversationId() resets it so a fresh conversation starts at default.
