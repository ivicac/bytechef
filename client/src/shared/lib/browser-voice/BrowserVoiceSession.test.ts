 ...
 *
 * Task 22 adds the `sessionId`/`outputSampleRate` adoption, terminal `session_end` handling, and the
 * reconnect-with-resume loop — those tests drive `start()` for real (via `startWithoutAudio`), which needs
 * the mic/AudioContext/AudioWorklet globals stubbed out since jsdom doesn't implement them.
 *
 * Task 22/23 fix round 1 adds coverage for: stop() racing an in-flight mintToken(), a single terminal error
 * event (no onerror+onclose double-fire), lastServerError not poisoning a reconnect-eligible close forever,
 * a mintToken() failure during reconnect counting as one attempt (not an immediate give-up), and normalizing
 * an `event: 'error'` frame to `type: 'error'` for consumers.
 */
...
/** Stubs the mic/AudioContext/AudioWorklet globals `start()` needs, since jsdom doesn't implement them. */
