...
export const buildAiHubSubscriber = ({
    ...
    return {
        ...
        onToolCallResultEvent: ({event}) => {
            ...
            const errorResult = parseJson<ToolCallErrorResultI>(toolResultContent, 'tool-result error envelope');
            ...
            const parsedAny = parseJson<unknown>(toolResultContent, 'tool-result generic payload');

            aiChatToolCallStore.getState().completeToolCall(event.toolCallId, parsedAny ?? toolResultContent, isError);
            ...
            if (toolCallName === 'openResourceTab') {
                const resourceResult = parseJson<{type?: unknown}>(toolResultContent, 'openResourceTab result');
                ...
                if (!legacyToolCallName) {
                    surfaceTabOpenFailure(toolCallName, 'unparseable result', toolResultContent);
            ...
            if (dispatchToolCallName === 'openFileTab') {
                const raw = parseJson<unknown>(toolResultContent, 'openFileTab result');

                if (raw === null) {
                    surfaceTabOpenFailure(dispatchToolCallName, 'unparseable result', toolResultContent);
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openWorkflowTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openDataTableTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openKnowledgeBaseTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openSkillTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openCustomComponentTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openCodeWorkflowTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, 'openAiAgentTab result');
                ...
                const raw = parseJson<unknown>(toolResultContent, `${toolCallName} result`);
                ...
                const parsed = parseJson<RunChatWorkflowResultI>(toolResultContent, 'runChatWorkflow result');
