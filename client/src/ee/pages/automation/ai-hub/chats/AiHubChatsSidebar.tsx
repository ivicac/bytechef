...
const ChatItem = ({
    ...
    return (
        <div>
            <div
                className={twMerge(
                    'group flex w-full items-center justify-between rounded-md py-1.5 pl-2 text-sm text-sidebar-foreground hover:bg-accent hover:text-sidebar-accent-foreground',
                    isCurrent && 'bg-accent text-sidebar-accent-foreground'
                )}
            >
                <button
...
const AiHubChatsSidebar = () => {
    ...
    const {data: sharedChats} = useAiHubSharedChatsQuery(currentWorkspaceId, currentEnvironmentId, sharingEnabled);
    ...
    return (
        <div className="flex flex-col gap-2 pb-2">
            {/*
             * Top-level menu block: New Chat / More. Aligned at px-2 so labels land at 8 (LayoutContainer
             * body px-2) + 8 (item px-2) = 16px from the sidebar edge — matches the px-4 padding on the
             * "AI Hub" Header above.
             *
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <button
                            ...
                            className={twMerge(
                                '-mr-1.5 inline-flex size-7 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground',
             ...
             * and the menu items above. Without it, the Input sits at the body px-2 edge, 8px in.
             */}
            ...
            {sharingEnabled && sharedChats && sharedChats.length > 0 && (
                <>
                    ...
                    {filteredSharedChats.length === 0 ? (
                        <div className="px-2 py-4">
                            <span className="text-xs text-muted-foreground">No shared chats match your search.</span>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-0.5">
