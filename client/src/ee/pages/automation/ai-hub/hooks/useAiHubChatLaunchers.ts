...
export const useAiHubChatLaunchers = (): AiHubChatLaunchersI => {
    ...
    // Agent chats are a disjoint set from workflow chats, not a subset: agent chats are listed by
    // `workspaceChatAgents`, and `workspaceChatWorkflows` excludes generated agent workflows by their
    // `project_workflow.type`. So the two cascades never show the same row twice.
    ...
    // better in the chat list than the "project — workflow" form workflow chats use (the generated agent
    // workflow is an implementation detail the user should never see by name).
