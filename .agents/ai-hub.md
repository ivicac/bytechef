...
- `AGENT_CHAT` (ordinal 2) — bound to an **AI Agent's** generated workflow. Mechanically
  ...
  generated `AI_AGENT` workflow behind it. Before it existed the client re-derived
...
those workflows are `ProjectWorkflowType.AI_AGENT` rows of the agents' projects, which
`workspaceChatWorkflows` filters out by type, so the two cascades are disjoint. A disabled or missing deployment drops the agent from the listing entirely —
...
**One path per capability.** A surface that registers a subagent must not also register the raw tool
that does that subagent's job — otherwise the same write can happen with or without the subagent's
procedure behind it. `updateMcpProjectWorkflowParameters` is filtered out wherever `configureMcpServer`
is present (AI Hub, management MCP, the MCP Servers panel). Workflow content is enforced by type
rather than by filter: every surface that registers `buildWorkflow` (the AI Hub BUILD tool-search
catalog, the management MCP server, the Projects panel) takes `ProjectWorkflowLifecycleTools` —
reads, `deleteWorkflow`, `saveWorkflowTestConnection` and an empty-only `createProjectWorkflow(projectId,
name, description?)` — instead of `ProjectWorkflowTools`, whose `updateWorkflow` and definition-accepting
create only the workflow-authoring subagents register. `WorkflowContentSinglePathTest` (server-app, the
one classpath that sees all three modules) fails if any of those surfaces injects `ProjectWorkflowTools`
again. The prompts, plugin skills and generated `mcp-instructions.md` route every such change through
the subagent, so none carries a copy of its playbook to drift.
