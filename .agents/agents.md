<!-- Extracted from CLAUDE.md so the agent-facing reference does not sit in every prompt.
     ...
     client/src/pages/automation/agents, client/src/pages/automation/project-deployments). -->
...
ByteChef workflow definition behind the scenes. An agent is a member of an ordinary, user-visible project,
next to that project's workflows. Specs: `docs/superpowers/specs/2026-08-10-agents-design.md` (original
design) and `docs/superpowers/specs/2026-09-21-agents-under-projects-design.md` (agents moved into ordinary
projects). Plans (task-by-task history): `docs/superpowers/plans/2026-08-10-agents.md`,
`docs/superpowers/plans/2026-09-21-agents-under-projects.md`.
...
- **`ai_agent`** (`AiAgent.java`) — `name` (slug, `^[a-z0-9_-]{1,64}$`, enforced in `setName` AND by a DB
  ...
  text), `workspaceId`, `projectId` (the ordinary project the agent belongs to — see "Project
  membership" below), `projectWorkflowUuid` (the uuid of its generated `project_workflow` row), `uuid`, `settings` (nullable JSON — per-agent built-in-tool on/off switches, see "Built-in
  tool settings" below).
...
An agent belongs to an ordinary project (`createAgent` takes an optional `projectId`; without one it
creates a project named after the agent's title). A project can hold several agents and ordinary
workflows side by side. Each agent's generated workflow is a `project_workflow` row with
`type = ProjectWorkflowType.AI_AGENT`, found by `ai_agent.project_workflow_uuid`; ordinary workflows are
`WORKFLOW` rows. Every ordinary workflow surface filters on `type = WORKFLOW` — project workflow lists,
call-workflow options, MCP tool pickers, search, duplicate/export and repository sync — and
`ProjectWorkflowFacade`'s update/delete/duplicate/export refuse an `AI_AGENT` row: the generated workflow
is edited only through the agent (the facade writes it with the raw `WorkflowService`). Visibility is the
project's; there is no per-agent visibility. Deleting the project deletes its agents through the
`ProjectDeleteEventListener` SPI (`AiAgentProjectDeleteEventListener` → `deleteProjectAgents`), refused
while an agent outside the project uses one of them as a sub-agent.
...
keeps the draft always current, but publishing is a **separate, explicit** action — and it is the
**project's** publish. There is no per-agent publish: publishing the project (Publish in the agent header,
the project header, or the `publishProject` MCP tool) publishes every agent in it.
`ProjectServiceImpl.publishProject` calls the `ProjectPublishPreListener` SPI first;
`AiAgentProjectPublishPreListener` skips projects without agents and otherwise calls
`AiAgentFacade.prepareProjectPublish`, which validates every agent of the project (exactly one MODEL;
connectionRequired channels have a connection; a channel row feeding a REQUIRED reply property has it
set; every SUB_AGENT target has a PUBLISHED version) and regenerates its draft before the snapshot. A
failing agent blocks the project publish. Each publish bumps `lastProjectVersion` and duplicates every
workflow into a **new** workflow row — a published agent accumulates one workflow row per published
version, never mutated in place.
...
An agent is deployed by deploying its project: the ordinary Project Deployments page and
`ProjectDeploymentDialog`. There is no Agent Deployments page. A deployment row renders
`ProjectDeploymentAgentChannels` → `AgentDeploymentChannelList` for the agents it contains (channel
list, static webhook URLs), fed by the `aiAgentDeployments(workspaceId)` GraphQL query →
`AiAgentFacade.getAgentDeployments`, which looks up each visible agent's `projectId` per `Environment`
via `projectDeploymentService.fetchProjectDeployment(projectId, environment)` and narrows each row to
the agent's own workflow. Per-agent enablement is the existing `ProjectDeploymentWorkflow.enabled` flag
on the agent's workflow; deployment tags are the project deployment's own.
...
Agents live under Projects: the Projects page's **Workflows | Agents | Data Syncs** tab strip
(`/automation/projects`, the Agents tab reached via the `?agents=all|scheduled` filter — the agent list
with a project badge per row, sidebar filtering by project and tag) and, inside a project, the same
three tabs on `ProjectsLeftSidebar` leading to `/automation/projects/:projectId/agents/:agentId`
(`AgentDetailHeader`, `AgentDetailContent`, `AgentTestChatPanel`). Components stay in
`client/src/pages/automation/agents`; deployment channels are in
`client/src/pages/automation/project-deployments`. Non-obvious invariants:
...
- **Version history is the project's.** The header's history button opens the SAME
  `ProjectVersionHistorySheet` the project header uses (it gained a `title` prop for this), fed by
  `aiAgentVersions(id)` → `AiAgentFacade.getAgentVersions` → the agent's project's `ProjectVersion`
  rows, newest first, mapped client-side into the sheet's REST shape. Fetched only while the sheet is
  open. Publish in the agent header publishes the project, through the project header's own
  `PublishPopover`, so a publish carries the description that makes that history worth reading.
- **Export/import move configuration, not state.** `exportAiAgent(id)` returns a JSON document
  ...
  `importAiAgent(workspaceId, json)` builds a new agent from one, through `createAgent` so it gets a
  project (the chosen target or a new one), permanent channels and draft workflow like any other. Deliberately lossy in two
...
- **An agent deployment IS a `ProjectDeployment`.** Same row, same id, same REST endpoints — the
  Project Deployments page shows the agent's channels on its project's deployment row; there are no
  agent-specific deployment tags.
...
`AgentDetailHeader.tsx` (rendered by the project agent route), with a `useCopilotPostTurnRegistry` registration that
...
attempt); or the deployment is distributed EE, where the workspace-verification lookup is unimplemented
and the recorder fails closed. In every one of these
...
two rows of one type can reply with their own row-configured values) stored as a
`ProjectWorkflowType.AI_AGENT` row of an ordinary project, next to that project's workflows and kept out of
every ordinary workflow surface by the `type` column. **Channels are declared by components**: a trigger
...
branch. Every save regenerates the draft workflow; publishing the project (there is no per-agent publish)
...
live in an `ai_agent_tag` join table, the `project_tag` shape. Client: the Agents tab of the Projects
page's **Workflows | Agents | Data Syncs** tab strip (`/automation/projects?agents=all|scheduled`) and
the project agent route (`/automation/projects/:projectId/agents/:agentId`), components in
`src/pages/automation/agents`; agent channels show on the Project Deployments page. See
